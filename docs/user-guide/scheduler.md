...
!!! note "Optional component"
    The scheduler is a standalone module (`forgent.scheduler`). It is not wired into the default `Forgent` / `ForgentSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `forgent scheduler start`.
...
The `forgent scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
forgent scheduler list --status active

# Paused tasks
forgent scheduler list --status paused
```
...
forgent scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
forgent scheduler resume a3f9b12c4d8e
```
...
forgent scheduler cancel a3f9b12c4d8e
```
...
forgent scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.forgent/scheduler.db")  # (1)!

# Wire in a ForgentSystem for task execution
...
4. Starts a daemon thread named `"forgent-scheduler"`. Daemon threads exit automatically when the main process exits.
...
forgent ask --agent orchestrator \
...
Scheduler settings live in the `[scheduler]` section of `~/.forgent/config.toml`.

```toml title="~/.forgent/config.toml"
[scheduler]
enabled = false
db_path = "~/.forgent/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.forgent/scheduler.db`     | SQLite database path                       |
