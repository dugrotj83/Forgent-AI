...
ForgentAI ships with two benchmarks:
...
from forgent.bench._stubs import BenchmarkResult
...
from forgent.bench.latency import LatencyBenchmark
from forgent.bench.throughput import ThroughputBenchmark
...
forgent bench run -n 50

# Run only the latency benchmark
forgent bench run -b latency

# Run only the throughput benchmark with 20 samples
forgent bench run -b throughput -n 20

# Specify model and engine
forgent bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
forgent bench run --json

# Write JSONL results to a file
forgent bench run -o results.jsonl

# Combine options
forgent bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from forgent.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
forgent bench run -b context_length
```
