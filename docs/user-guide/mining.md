...
ForgentAI can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
ForgentAI falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run forgent mine init --cuda-visible-devices 0
```
...
- `forgent mine models` lists Pearl model support status.
- `forgent mine inspect-model` checks a Pearl model artifact before GPU launch.
- `forgent mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `forgent mine init` writes the local mining config and resolves the image.
- `forgent mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `forgent mine stop` stops the provider and removes the sidecar.
- `forgent mine status` reads live gateway metrics.
- `forgent mine attach` writes a sidecar for a miner you launched manually.
- `forgent mine logs` prints the Docker container log tail.
- `forgent mine validate-model` probes the active vLLM miner and gateway before
...
ForgentAI only lists Pearl-compatible models published by the Pearl Research
...
pass the ForgentAI H100/H200 validation run.

When validating a Pearl org model on a mining host, run:

```bash
forgent mine inspect-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned

forgent mine validate-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned \
...
v1 is solo mining only. ForgentAI does not take fees, custody funds, generate
wallet keys, run pools, or operate `pearld`. Users provide their own Pearl node
and payout address.

Unsupported in this PR:

- Pool mining and the future 20% ForgentAI fee model
- AMD GPU mining and non-Pearl backends
- RTX 4090 or other non-Hopper NVIDIA GPUs
- Wallet generation or transaction signing inside ForgentAI
