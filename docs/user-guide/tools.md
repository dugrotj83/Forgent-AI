...
from forgent.tools._stubs import ToolSpec
...
# Execute a tool call
from forgent.core.types import ToolCall
...
```python
from forgent.tools._stubs import build_tool_descriptions
...
```python
from forgent.tools.retrieval import RetrievalTool
from forgent.memory.sqlite import SQLiteMemory
...
```python
from forgent.tools.llm_tool import LLMTool
...
```python
from forgent.tools.file_read import FileReadTool
...
```python
from forgent.scheduler.tools import ScheduleTaskTool
from forgent.scheduler.scheduler import TaskScheduler
from forgent.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.forgent/scheduler.db")
scheduler = TaskScheduler(store=store, system=forgent_system)
scheduler.start()
...
from forgent.tools._stubs import BaseTool, ToolSpec
from forgent.core.types import ToolResult
...
```bash
forgent ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
forgent ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
forgent ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
