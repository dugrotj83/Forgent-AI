...
```bash
python examples/forgent/spec_search_quickstart.py
```
...
`forgent.learning.spec_search.composite_reward.score_batch`.
...
`configs/forgent/examples/spec-search-quickstart.toml`. Copy it to
`~/.forgent/config.toml` (or set `FORGENT_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `forgent.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `forgent.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `forgent.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `forgent.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `forgent.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/forgent/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `forgent.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from forgent.evals.datasets.adp import ADPDataset
from forgent.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.forgent/traces.db")
...
`src/forgent/evals/backends/forgent_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.forgent/traces.db` regardless of caller
...
- `examples/forgent/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/forgent/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/forgent/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/forgent/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/forgent/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/forgent/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/forgent/learning/spec_search/external_adapter.py` — corpus → trace adapter.
