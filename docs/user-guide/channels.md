...
The channels module lets ForgentAI send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from forgent.channels.telegram import TelegramChannel
...
from forgent.channels._stubs import ChannelMessage
from forgent.channels.discord_channel import DiscordChannel
...
from forgent.channels.slack import SlackChannel
...
from forgent.core.events import EventBus, EventType
from forgent.channels.telegram import TelegramChannel
...
The `forgent channel` subcommand group provides quick access to channel operations.
...
forgent channel send telegram "Build completed successfully"
```
...
When `forgent serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.forgent/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.forgent/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from ForgentAI!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  ForgentAI
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. ForgentAI sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since ForgentAI runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from forgent.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from ForgentAI!")
```
...
forgent channel status --channel-type sendblue

# Send a message
forgent channel send sendblue "+15551234567" "Hello from Forgent!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `forgent serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.forgent/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from forgent.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Forgent",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.forgent/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.forgent/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Forgent"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.forgent/whatsapp_baileys_bridge/auth"
assistant_name = "Forgent"
...
- [API Reference: Channels](../api-reference/forgent/channels/index.md) -- full class and type signatures
