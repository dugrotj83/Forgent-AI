...
| **Overlay** | Sidecar file at `~/.forgent/learning/skills/` storing optimized descriptions and few-shot examples |
...
forgent skill install hermes:apple-notes

# Bulk install a category
forgent skill sync hermes --category research

# Run a skill directly
forgent skill run math-solver -a expression="41 + 82"

# See skill details
forgent skill info research-and-summarize
```
...
forgent skill sync hermes --category research
forgent skill sync hermes --category coding
forgent skill sync hermes  # everything (~150 skills)
```
...
forgent skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
forgent skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
forgent skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
forgent skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.forgent/config.toml` for automatic syncing:
...
forgent skill sources

# Update all configured sources
forgent skill update
```
...
ForgentAI can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
forgent skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.forgent/skills/discovered/
forgent skill discover
```

Discovered skills land in `~/.forgent/skills/discovered/` and automatically appear in `forgent skill list` on the next session.
...
forgent optimize skills --dry-run

# Run DSPy optimization
forgent optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
forgent optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
forgent skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.forgent/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
|------|--------|-------------|---------|
| **Bundled** | Ships with ForgentAI | Implicit trust | Full access within declared capabilities |
...
```bash
forgent skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.forgent/skills/"  # where skills are installed
...
2. **User** `~/.forgent/skills/`
3. **Bundled** (shipped with ForgentAI)

## CLI Reference

| Command | Description |
|---------|-------------|
| `forgent skill list` | List installed skills |
| `forgent skill info <name>` | Show detailed skill information |
| `forgent skill run <name> [-a key=value]` | Execute a skill directly |
| `forgent skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `forgent skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `forgent skill sources` | List configured skill sources |
| `forgent skill update` | Pull latest from configured sources |
| `forgent skill remove <name>` | Remove an installed skill |
| `forgent skill search <query>` | Search the skill index |
| `forgent skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `forgent skill show-overlay <name>` | Inspect optimization output for a skill |
| `forgent optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `forgent bench skills [--condition C]` | Run the PinchBench skills benchmark |
