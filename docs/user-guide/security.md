...
ForgentAI includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
=== "Warn mode (default)"
    ...
    from forgent.security.guardrails import GuardrailsEngine
    from forgent.security.types import RedactionMode
...
=== "Block mode"
    ...
    from forgent.engine.ollama import OllamaEngine
    from forgent.security.guardrails import GuardrailsEngine, SecurityBlockError
    from forgent.security.types import RedactionMode
    from forgent.core.types import Message, Role
...
from forgent.security.guardrails import GuardrailsEngine
from forgent.security.scanner import SecretScanner
from forgent.security.types import RedactionMode
...
from forgent.security.scanner import PIIScanner
...
from forgent.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.forgent/audit.db", bus=bus)
...
import time
from forgent.security.audit import AuditLogger
from forgent.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.forgent/audit.db")
...
Security settings live in the `[security]` section of `~/.forgent/config.toml`.
...
secret_scanner = true
pii_scanner = true
audit_log_path = "~/.forgent/audit.db"
...
| `audit_log_path` | `str` | `~/.forgent/audit.db` | Path to the SQLite audit log |
...
import re
from forgent.security._stubs import BaseScanner
from forgent.security.types import ScanFinding, ScanResult, ThreatLevel
...
- [API Reference: Security](../api-reference/forgent/security/index.md) — full class and function signatures
