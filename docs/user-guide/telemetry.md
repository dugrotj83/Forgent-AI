...
ForgentAI has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.forgent/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.forgent/telemetry.db")
...
forgent telemetry stats -n 5          # Top 5 models only

# Export records
forgent telemetry export              # JSON to stdout
forgent telemetry export -f csv       # CSV to stdout
forgent telemetry export -o data.json # JSON to file
forgent telemetry export -f csv -o metrics.csv

# Clear all records
forgent telemetry clear               # With confirmation prompt
forgent telemetry clear --yes         # Without confirmation
```
