...
This writes a pre-configured `~/.forgent/config.toml` for the deep research agent.
...
ForgentAI chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
forgent ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
forgent ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
forgent memory index ~/Downloads/reports/
forgent memory index ./notes.md

# Search memory directly
forgent memory search "project timeline"
forgent memory search -k 20 "budget estimates"
...
The preset writes this to `~/.forgent/config.toml`:
...
forgent ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
forgent ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
forgent ask "What meetings did I have with Alice last month?"

# Extract structured data
forgent ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
forgent ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
forgent memory index ./report.pdf

# Custom chunk size for long documents
forgent memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
forgent memory index ~/Papers/*.pdf
```
...
```bash
curl -s https://example.com/article | forgent memory index --stdin --source "example.com"
```

### Code repositories

```bash
forgent memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `forgent memory index`. Check indexed content with `forgent memory stats`.
