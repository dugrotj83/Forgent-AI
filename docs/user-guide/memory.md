...
- **Persistence:** SQLite database file (default: `~/.forgent/memory.db`)
...
```python
from forgent.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from forgent.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `forgent.tools.storage.*` imports.
...
[Source: docs/intro.md] ForgentAI is a modular AI framework...
...
forgent memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
forgent memory search "machine learning"
