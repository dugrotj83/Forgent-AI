...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible ForgentAI setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize ForgentAI

```bash
git clone https://github.com/dugrotj83/Forgent-AI.git
cd Forgent-AI
uv sync
forgent init --preset chat-simple
```

### 3. Ask a question

```bash
forgent ask "What is quantum computing?"
```
...
```bash
# Single question
forgent ask "Explain the difference between TCP and UDP"
...
forgent ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
forgent ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
forgent ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.forgent/config.toml` or override per-query:

```bash
forgent ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `forgent serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/dugrotj83/Forgent-AI/releases)
...
${EDITOR:-nano} ~/.forgent/config.toml
...
forgent ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
FORGENT_MODEL=qwen3.5:9b forgent ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`forgent serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
