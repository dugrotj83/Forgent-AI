...
### 1. Install and set up ForgentAI
...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.forgent/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.forgent/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.forgent/connectors/news_rss.json << 'EOF'
{"feeds": [
...
export OPENAI_API_KEY="sk-prfg-..."
```

### 6. Run your first digest

```bash
CARTESIA_API_KEY="sk_car_..." forgent digest --fresh
```
...
```bash
forgent digest --fresh          # Generate a new digest now
forgent digest                  # Show today's cached digest
forgent digest --text-only      # Print text without audio
forgent digest --history        # Show past digests
forgent digest --schedule "0 6 * * *"   # Set daily schedule
forgent digest --schedule off   # Disable schedule
forgent digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Forgent (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/forgent/prompts/personas/{name}.md`. The default `forgent` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.forgent/connectors/news_rss.json`:
...
```bash
forgent serve  # Start the server
...
**"No digest for today"** — Run `forgent digest --fresh` to generate one.

**Empty sections** — Check connector status with `forgent connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
