...
This writes a pre-configured `~/.forgent/config.toml` for the operative agent with scheduling support.
...
forgent scheduler status <task-id>

# Pause / resume / delete a task
forgent scheduler pause <task-id>
forgent scheduler resume <task-id>
forgent scheduler delete <task-id>

# Run a task immediately (outside its schedule)
forgent scheduler run <task-id>

# Stop the scheduler daemon
forgent scheduler stop
```
...
- **All data stays local**: State is stored in `~/.forgent/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `forgent scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `forgent scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `forgent memory stats`. Clear old entries with `forgent memory clear --before 2026-01-01` if needed.
