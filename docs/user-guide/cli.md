...
ForgentAI provides a command-line interface through the `forgent` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
forgent --version   # Print the ForgentAI version
forgent --help      # Show top-level help with all subcommands
```

## `forgent init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.forgent/config.toml`.

```bash
forgent init           # Interactive — refuses to overwrite existing config
forgent init --force   # Overwrite existing config without prompting
```
...
```bash
forgent ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
forgent ask --agent orchestrator "What is 2+2?"
forgent ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
forgent ask --agent simple "Hello"
```
...
```bash
# Basic query
forgent ask "What is machine learning?"

# Specify a model
forgent ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
forgent ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
forgent ask --json "Hello"

# Disable memory context injection
forgent ask --no-context "Tell me about Python"

# Set maximum token generation
forgent ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
forgent ask -i screenshot.png "What is shown in this image?"

# Send multiple images (the flag is repeatable)
forgent ask -i chart-a.png -i chart-b.png "Compare these two charts"

# Capture the current screen and ask about it
forgent ask --screen "Summarize what's on my screen"
```
...
the `FORGENT_NUM_CTX` environment variable (default `16384`):

```bash
FORGENT_NUM_CTX=8192 forgent ask --screen "What's on my screen?"
```

!!! note "Keep vision on-device"
    Images are sensitive. ForgentAI prints a privacy warning before sending
...
Access Pearl's native node, wallet, and RPC tools from the ForgentAI CLI.

```bash
forgent pearl doctor
forgent pearl node -- <pearld args>
forgent pearl wallet -- <oyster args>
forgent pearl ctl -- <prlctl args>
forgent pearl address
```

All Pearl wrapper commands use the `forgent pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| ForgentAI command | Pearl binary | Use |
|--------------------|--------------|-----|
| `forgent pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `forgent pearl node` | `pearld` | Run the Pearl full node |
| `forgent pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `forgent pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `forgent pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `forgent memory index <path>`

Index documents from a file or directory into the memory store.

```bash
forgent memory index ./docs/
forgent memory index ./notes.md
forgent memory index ./data/ --chunk-size 256 --chunk-overlap 32
forgent memory index ./docs/ --backend sqlite
```
...
### `forgent memory search <query>`

Search the memory store for relevant document chunks.

```bash
forgent memory search "machine learning basics"
forgent memory search -k 10 "neural networks"
forgent memory search --backend faiss "embeddings"
```
...
```bash
forgent memory stats
forgent memory stats --backend sqlite
```
...
```bash
forgent telemetry stats
forgent telemetry stats -n 5    # Show top 5 models
```
...
### `forgent telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
forgent telemetry export                          # JSON to stdout
forgent telemetry export --format csv             # CSV to stdout
forgent telemetry export --format json -o data.json  # JSON to file
forgent telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `forgent telemetry clear`

Delete all telemetry records from the database.

```bash
forgent telemetry clear         # Interactive confirmation
forgent telemetry clear --yes   # Skip confirmation
```
...
```bash
forgent bench run                               # Run all benchmarks, 10 samples
forgent bench run -n 20                         # 20 samples per benchmark
forgent bench run -b latency                    # Only the latency benchmark
forgent bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
forgent bench run -o results.jsonl              # Write JSONL results to file
forgent bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
### `forgent channel send`

Send a message to a specific channel.

```bash
forgent channel send slack "Hello from Forgent!"
forgent channel send discord "Build complete"
```
...
```bash
forgent serve                                 # Default host/port from config
forgent serve --port 8000                     # Custom port
forgent serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
forgent serve --model qwen3:8b                # Specify default model
forgent serve --agent orchestrator            # Route requests through an agent
```
...
`forgent` subcommand for it. Construct a `SpecSearchOrchestrator`
directly from `forgent.learning.spec_search.orchestrator` and call
`.run(trigger)` with a trigger from
`forgent.learning.spec_search.triggers`. See
