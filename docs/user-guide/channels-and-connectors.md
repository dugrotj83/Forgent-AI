...
ForgentAI has two types of integrations:
...
```json
{
    "display_information": { "name": "ForgentAI" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "ForgentAI", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "ForgentAI" |
...
Slack serves two purposes in ForgentAI:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [ForgentAI icon](https://github.com/dugrotj83/Forgent-AI/blob/main/assets/forgent-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **ForgentAI** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "ForgentAI"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @ForgentAI` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @ForgentAI` in the channel |
...
3. **Connect in ForgentAI:**
   ...
   - CLI: `uv run forgent connect notion`
...
3. **Connect in ForgentAI:**
   ...
   - CLI: `uv run forgent connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the ForgentAI desktop app
...
3. **Connect in ForgentAI:**
   ...
   - CLI: `uv run forgent connect outlook`
...
2. **Connect in ForgentAI:**
   - Desktop/Browser: Agents → Channels tab → Obsidian → paste the vault path
   - CLI: `uv run forgent connect obsidian --path /path/to/vault`
...
4. **Connect in ForgentAI:**
   ...
   - CLI: `uv run forgent connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run forgent deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.forgent/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.forgent/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
