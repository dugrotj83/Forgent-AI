...
This writes a pre-configured `~/.forgent/config.toml` for the code assistant.
...
```bash
forgent ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
forgent ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
forgent ask "Read main.py and explain the architecture"

# Fix a bug
forgent ask "Find and fix the bug in test_utils.py"

# Run tests
forgent ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
forgent ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
```bash
# Write a new script
forgent ask "Write a Python script that converts YAML to JSON"

# Explain existing code
forgent ask "Read src/forgent/core/events.py and explain the EventBus pattern"

# Debug a failing test
forgent ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
forgent ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
forgent ask "Read src/forgent/tools/calculator.py and write unit tests for it"

# Shell tasks
forgent ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`forgent chat`).
- For stronger isolation, use the sandboxed agent: `forgent ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `forgent`. Use `cd /path && command` in your prompt if needed, or run `forgent` from the project directory.
