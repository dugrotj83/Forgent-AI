...
The ForgentAI evaluation framework (`forgent.evals`) measures model **correctness and accuracy** on academic datasets. It ships inside the main `forgent` package (at `src/forgent/evals/`) and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    ForgentAI has two distinct measurement systems that complement each other:

    | System | Module | Measures | Entry Point |
    |--------|--------|----------|-------------|
    | **Evaluations** | `forgent.evals` | Correctness on academic datasets (accuracy, pass rate) | `forgent eval` |
    | **Benchmarks** | `forgent.bench` | Engine performance (latency, throughput) | `forgent bench` |
...
The evaluation framework is part of the main `forgent` package — no separate install or extra is required. The standard dev setup is enough:

```bash
uv sync --extra dev
```

The framework's core dependencies (`click`, `datasets`, `rich`) are base dependencies of `forgent`. Two optional extras enable experiment tracking integrations:
...
!!! note "Python version requirement"
    Python 3.10 requires the `tomli` package for TOML config parsing. `forgent` declares it as a conditional dependency, so it is installed automatically.
...
| Command | Surface |
|---------|---------|
| `forgent eval {list,run,compare,report}` | Canonical CLI. `run` covers the common options; `compare` and `report` post-process result files. |
| `python -m forgent.evals {list,run,run-all,summarize,reparse-judge}` | Full research surface, including judge configuration, the agentic runner, and episode mode. |

The `forgent-eval` console script is an alias for `python -m forgent.evals` — same commands, same options. This guide uses `forgent eval` wherever its option set suffices and the module form for research-only options.

---

## Datasets

The framework ships with **40 registered benchmarks** covering academic reasoning, agentic tasks, coding, retrieval, conversation quality, and practical use-case benchmarks. Datasets are grouped by category below; `uv run python -m forgent.evals list` prints the authoritative registry.

### Use-Case Benchmarks

These benchmarks evaluate models on practical tasks that mirror real ForgentAI use cases.
...
```bash
uv run forgent eval run --config src/forgent/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run forgent eval run --config src/forgent/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **forgent-direct** | `forgent-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **forgent-agent** | `forgent-agent` | Agent-level inference with tool calling. Uses `ForgentSystem.ask()` with the specified agent and tools. |
...
Use `forgent-direct` for most evaluations. Use `forgent-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.

The `hermes` and `openclaw` backends shell out to external agent frameworks and need an OpenAI-compatible endpoint for their model calls: pass `--base-url`/`--api-key`, set the `FORGENT_BACKEND_BASE_URL`/`FORGENT_BACKEND_API_KEY` environment variables, or add a `[backend.external]` section to your config (see [Config Reference](#backendexternal)).

!!! note "TerminalBench Native"
    `forgent eval run --backend` additionally accepts `terminalbench-native`, a Docker-based execution backend used by the TerminalBench Native benchmark.
...
```bash
uv run python -m forgent.evals list
```
...
`forgent eval list` prints a similar table but currently shows a curated subset of the registry; the module form above is the authoritative listing.

### Run a single benchmark

```bash
# Evaluate qwen3:8b on SuperGPQA (engine-level, 10 samples)
uv run forgent eval run -b supergpqa -m qwen3:8b -n 10

# Evaluate GPT-5 Mini on GAIA using the agent backend with tools
uv run forgent eval run -b gaia -m gpt-5-mini --backend forgent-agent \
    --agent orchestrator --tools calculator,file_read -n 50

# Run FRAMES with the vLLM engine, write output to a file
uv run forgent eval run -b frames -m llama3:70b -e vllm \
    -o results/frames_llama70b.jsonl

# Run WildChat with a higher temperature for chat quality
uv run forgent eval run -b wildchat -m qwen3:8b --temperature 0.7 -n 100
```

#### `forgent eval run` option reference
...
| `--backend` | | choice | `forgent-direct` | `forgent-direct`, `forgent-agent`, `hermes`, `openclaw`, or `terminalbench-native` |
| `--base-url` | | str | — | OpenAI-compatible endpoint URL (env: `FORGENT_BACKEND_BASE_URL`) |
| `--api-key` | | str | — | API key for the endpoint (env: `FORGENT_BACKEND_API_KEY`) |
| `--agent` | | str | — | Agent name for `forgent-agent` backend (e.g., `orchestrator`) |
...
#### Research-only options (`python -m forgent.evals run`)

The module CLI accepts everything above plus research-grade options that `forgent eval run` does not expose:
...
Note: the module CLI's `--backend` choice covers `forgent-direct`, `forgent-agent`, `hermes`, and `openclaw`; `terminalbench-native` as a backend is available via `forgent eval run` and TOML configs.
...
```bash
uv run python -m forgent.evals run-all -m qwen3:8b

# With options
uv run python -m forgent.evals run-all -m gpt-5-mini -n 100 --output-dir results/gpt5mini/
```
...
```bash
uv run python -m forgent.evals summarize results/supergpqa_qwen3-8b.jsonl
```
...
`forgent eval` adds two post-processing commands for result files:

```bash
# Side-by-side metric comparison across runs
uv run forgent eval compare results/supergpqa_qwen3-8b.jsonl results/supergpqa_gpt-5-mini.jsonl

# Detailed report (accuracy, latency, cost, per-subject breakdown) for one run
uv run forgent eval report results/supergpqa_qwen3-8b.jsonl
```
...
If you already have an OpenAI-compatible server running — `forgent serve`, vLLM, SGLang, llama.cpp's server, or a hosted endpoint — point an eval directly at it with `--base-url` and `--api-key`:

```bash
# A vLLM server is already serving Qwen/Qwen3-8B on a GPU node:
#   vllm serve Qwen/Qwen3-8B --port 8000
uv run forgent eval run -b supergpqa -m Qwen/Qwen3-8B \
    --base-url http://gpu-node:8000/v1 \
    --api-key local-key \
    -n 50
```

The `-m` value must match a model id the server reports at `GET /v1/models`. Both flags fall back to the `FORGENT_BACKEND_BASE_URL` and `FORGENT_BACKEND_API_KEY` environment variables, so CI jobs can set them once:

```bash
export FORGENT_BACKEND_BASE_URL=http://gpu-node:8000/v1
export FORGENT_BACKEND_API_KEY=local-key
uv run forgent eval run -b gaia -m Qwen/Qwen3-8B --backend forgent-agent -n 25
```
...
!!! tip "Engine-level alternative for vLLM"
    ...
    ```bash
    VLLM_HOST=http://gpu-node:8000 uv run python -m forgent.evals run \
...
```bash
uv run forgent eval run --config src/forgent/evals/configs/full-suite.toml
```
...
```toml title="src/forgent/evals/configs/full-suite.toml"
...
```toml title="src/forgent/evals/configs/minimal.toml"
[[models]]
name = "qwen3:8b"
...
```toml title="src/forgent/evals/configs/single-run.toml"
[meta]
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `base_url` | str | `None` | OpenAI-compatible endpoint URL (env: `FORGENT_BACKEND_BASE_URL`) |
| `api_key` | str | `None` | API key for the endpoint (env: `FORGENT_BACKEND_API_KEY`) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | str | required | Any registered benchmark key (see `uv run python -m forgent.evals list`) |
| `backend` | str | `"forgent-direct"` | `forgent-direct`, `forgent-agent`, `hermes`, `openclaw`, or `terminalbench-native` |
...
| `agent` | str | `None` | Agent name for `forgent-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `forgent-agent` backend |
...
uv run python -m forgent.evals run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `forgent-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to ForgentAI inference and agents
