...
ForgentAI provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.
...
cd deploy/docker
cp .env.example .env
echo "FORGENT_API_KEY=$(forgent auth generate-key)" > .env   # or paste your own
```
...
`docker compose` reads `FORGENT_API_KEY` from `.env` (or your shell
...
|----------|-------|------------------------------------|
| `forgent` | 8000  | ForgentAI API server              |
...
1. **Builder stage** -- installs `uv` and the `forgent[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker build -t forgent:latest .
```

Run it standalone:

```bash
docker run -d -p 8000:8000 forgent:latest
```
...
```bash
docker build -f Dockerfile.gpu -t forgent:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 forgent:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the ForgentAI API server and an Ollama backend:
...
services:
  forgent:
    ...
    ports:
      - "8000:8000"
    environment:
      - FORGENT_ENGINE_DEFAULT=ollama
      - FORGENT_OLLAMA_HOST=http://ollama:11434
    depends_on:
      - ollama
...
The `forgent` service is configured through environment variables:

| Variable                      | Description                                             | Default                    |
|-------------------------------|---------------------------------------------------------|----------------------------|
| `FORGENT_ENGINE_DEFAULT`   | Inference engine backend to use                         | `ollama`                   |
| `FORGENT_OLLAMA_HOST`      | URL of the Ollama server (uses Docker service name)     | `http://ollama:11434`      |
...
The `forgent` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.forgent/config.toml`, which is `/root/.forgent/config.toml` in the container):
...
services:
  forgent:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.forgent/config.toml:ro
    environment:
      - FORGENT_ENGINE_DEFAULT=ollama
      - FORGENT_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire ForgentAI data directory:
...
The entrypoint is `forgent` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 forgent:latest \
...
### Available CLI Options for `forgent serve`
