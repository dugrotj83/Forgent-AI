...
ForgentAI follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in ForgentAI is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
ForgentAI is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Forgent(engine_key="ollama")  # Local Ollama server
...
ForgentAI **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `forgent init` and used as the default engine:

```bash
forgent init --force
# Detects hardware, writes ~/.forgent/config.toml with:
# [engine]
...
```bash
forgent telemetry stats          # Aggregated statistics
forgent telemetry export --json  # Export all records
```
...
ForgentAI provides a **clean Python API** through the `Forgent` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`forgent[server]`, `forgent[memory-colbert]`, etc.)
...
The API server (`forgent serve`) implements the **OpenAI chat completions API format**, making ForgentAI a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to ForgentAI:
...
ForgentAI requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of ForgentAI.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `forgent[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `forgent[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `forgent[memory-bm25]` | `rank-bm25` |
| API server | `forgent[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `forgent[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `forgent[inference-vllm]` | `vllm` |
| PDF ingestion | `forgent[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `forgent[channel-whatsapp-baileys]` | Node.js 22+ |
