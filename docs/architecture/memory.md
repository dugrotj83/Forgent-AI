...
- **Persistence:** Data persists across restarts in `~/.forgent/memory.db`
...
from forgent.tools.storage.faiss_backend import FAISSMemory
from forgent.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from forgent.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `forgent.tools.storage.*`.
...
```python
from forgent.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from forgent.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from forgent.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.forgent/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
