...
description: The five-primitive architecture behind ForgentAI — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
The sandbox module (`forgent.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in ForgentAI use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/forgent/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           ForgentConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              forgent ask -- query the assistant
        serve.py            forgent serve -- start API server
