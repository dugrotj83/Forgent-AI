...
This page traces the end-to-end journey of a user query through the ForgentAI system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: forgent ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: ForgentConfig (hardware, engine defaults)
...
ForgentAI supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
forgent ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.forgent/config.toml
```
...
- Returns a `ForgentConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.forgent/telemetry.db`.
...
The `Forgent` class in `sdk.py` provides the same query flow through a Python API:

```python
from forgent import Forgent

j = Forgent(model="qwen3:8b", engine_key="ollama")
