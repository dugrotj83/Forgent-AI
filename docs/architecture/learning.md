...
!!! note "Backward compatibility"
    The canonical locations are now `forgent.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `forgent.core.types` (for `RoutingContext`). The old `forgent.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `forgent.learning._stubs`.
...
```bash
forgent ask --router learned "What is the capital of France?"
```
...
```python
from forgent.learning.router import HeuristicRouter, build_routing_context
...
```python
from forgent.learning.router import build_routing_context
...
```python
from forgent.learning.trace_policy import TraceDrivenPolicy
from forgent.traces.analyzer import TraceAnalyzer
...
```python
from forgent.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from forgent.learning.sft_policy import SFTPolicy
```
...
```python
from forgent.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from forgent.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from forgent.learning.heuristic_reward import HeuristicRewardFunction
...
over ForgentAI's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `forgent-learning`
