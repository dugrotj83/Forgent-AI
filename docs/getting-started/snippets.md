---
title: Code Snippets
description: Copy-paste patterns for common ForgentAI tasks
---

# Code Snippets

Ready-to-use patterns for the most common ForgentAI tasks. Each snippet is self-contained and copy-pasteable.
...
Run with: `forgent compose run research_assistant "quantum computing advances"`

## API Server (1 command)

```bash
forgent serve --port 8000 --engine ollama --model qwen3:8b
```
...
```bash
docker build -t forgent .
docker run -p 8000:8000 forgent serve --host 0.0.0.0
```
