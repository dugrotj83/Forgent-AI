---
title: Configuration
description: Complete reference for ForgentAI configuration
---

# Configuration

ForgentAI uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
ForgentAI creates the `~/.forgent/` directory and populates it with a default config when you run `forgent init`.

## Relocating the ForgentAI directory

ForgentAI keeps **all** of its state — config, databases, caches, logs,
...
`~/.forgent`, but you can move it.

The root is resolved in priority order:

1. **`$FORGENT_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/forgent`** — used when `$XDG_DATA_HOME` is set (a single
   `forgent` directory nested under it, per the XDG Base Directory spec).
3. **`~/.forgent`** — the default. With no environment variables set, the
   resolved path is exactly this, so existing installs are untouched.

```bash
# Relocate the whole install + runtime tree at install time:
FORGENT_HOME=~/apps/forgent curl -fsSL https://github.com/dugrotj83/Forgent-AI/install.sh | bash

# Or for a single run / your shell profile:
export FORGENT_HOME=~/apps/forgent
```
...
!!! note "Migration"
    ...
    existing installs. If you set `FORGENT_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.forgent`, ForgentAI will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.forgent "$FORGENT_HOME"`.

`$FORGENT_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.forgent/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `forgent init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, ForgentAI automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, ForgentAI uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
forgent ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.forgent/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `forgent serve`.
...
```bash
forgent serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.forgent/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.forgent/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.forgent/skills/` | Directory where skills are installed. |
...
overlay_dir = "~/.forgent/learning/skills/"
```
...
| `overlay_dir` | string | `~/.forgent/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `forgent init`, ForgentAI probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.forgent/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure ForgentAI entirely from Python without a TOML file:

```python
from forgent import Forgent
from forgent.core.config import (
    AgentConfig,
    EngineConfig,
    IntelligenceConfig,
    ForgentConfig,
...
```python
j = Forgent(config_path="/path/to/my-config.toml")
```
...
ForgentAI respects the following environment variables:
