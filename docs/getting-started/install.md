...
|---|---|---|
| **macOS** | `curl -fsSL https://github.com/dugrotj83/Forgent-AI/install.sh \| bash` | [macOS install](macos.md) |
| **Linux** | `curl -fsSL https://github.com/dugrotj83/Forgent-AI/install.sh \| bash` | [Linux install](linux.md) |
| **WSL2 on Windows** | `curl -fsSL https://github.com/dugrotj83/Forgent-AI/install.sh \| bash` (run inside Ubuntu) | [WSL2 install](wsl2.md) |
| **Native Windows** | `irm https://github.com/dugrotj83/Forgent-AI/install.ps1 \| iex` | [Native Windows install](windows-native.md) |
| **Desktop GUI** | Download from the [latest release](https://github.com/dugrotj83/Forgent-AI/releases) | — |
...
!!! info "Install URL"
    ...
    so HTTPS always works. You may also see `https://forgent.ai/install.sh`
    referenced in older docs — that domain is community-operated and has had
    intermittent TLS issues ([#337](https://github.com/dugrotj83/Forgent-AI/issues/337)).
    The `github.com/dugrotj83/Forgent-AI` URL above is the canonical one.

About 3 minutes on a typical broadband connection. Type `forgent` to start chatting.
...
| Foreground | Clone ForgentAI repo | `~/.forgent/src/` |
| Foreground | Create Python 3.11 venv | `~/.forgent/.venv/` |
...
| Foreground | Write `config.toml` (auto-detected hardware + engine + model) | `~/.forgent/config.toml` |
| Foreground | Symlink `forgent` and `forgent-uninstall` | `~/.local/bin/` |
...
Re-running the curl line is safe. The installer reads `~/.forgent/.state/install-state.json` and skips completed steps. If your venv got nuked, re-running heals it.

## Cloud quick-path

If any of these env vars are set when you install or run `forgent init`, the installer/init proposes cloud as the default and writes the matching provider into `config.toml`:
...
|---|---|---|
| `FORGENT_HOME` | `$HOME/.forgent` | Install location. |
| `FORGENT_REPO_URL` | `https://github.com/dugrotj83/Forgent-AI.git` | Source repo for the clone step. |
...
Removes `~/.forgent/`, `~/.local/bin/forgent`, and `~/.local/bin/forgent-uninstall`. Leaves Ollama, uv, and the Rust toolchain in place (they may be used by other tools); the script prints removal hints.
...
```bash
~/.forgent/.scripts/install-rust.sh && ~/.forgent/.scripts/build-extension.sh
```
...
```bash
forgent doctor
~/.forgent/.scripts/pull-model.sh qwen3.5:9b
```
