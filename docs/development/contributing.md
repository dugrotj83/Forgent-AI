...
```bash
uv run forgent --version   # Should print 0.1.0
uv run forgent --help      # Show all subcommands
```
...
ForgentAI uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=forgent --cov-report=html
```
...
ForgentAI uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/forgent/`:

```
src/forgent/
    __init__.py                 # Package root, __version__
    sdk.py                      # Forgent class — high-level Python SDK

    core/                       # Shared infrastructure
        config.py               # ForgentConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # forgent ask
        init_cmd.py             # forgent init
        model.py                # forgent model list/info
        memory_cmd.py           # forgent memory index/search/stats
        telemetry_cmd.py        # forgent telemetry stats/export/clear
        bench_cmd.py            # forgent bench run
        serve.py                # forgent serve
```
...
try:
    import forgent.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
