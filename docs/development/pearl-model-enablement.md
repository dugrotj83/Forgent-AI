...
through Pearl's vLLM miner and ForgentAI.

ForgentAI can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
ForgentAI only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and ForgentAI-specific conversion
...
`forgent mine start`, vLLM `/v1/models`, ForgentAI inference routing, Pearl
gateway template refresh, and `forgent mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in ForgentAI until we have clean
...
2. Convert the target model.
   ...
   ForgentAI includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `forgent mine inspect-model` and
   `forgent mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `forgent mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `forgent mine status` parses those metrics.

5. Promote the model in ForgentAI.
...
```text
src/forgent/mining/_models.py
```

`forgent mine models` renders that registry. Planned models are visible to users
...
- `forgent mine inspect-model --model <pearl-model-id> --allow-planned`
- `forgent mine init --model <pearl-model-id>`
- `forgent mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `forgent ask "Say hello in one sentence."`
- `forgent mine status`
- `forgent mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `forgent mine validate-model --output` to the issue.
