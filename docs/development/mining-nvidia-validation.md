...
prove ForgentAI wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through ForgentAI.
...
- Docker container `forgent-pearl-miner` starts.
- `~/.forgent/runtime/mining.json` is written.
...
```bash
docker ps --filter name=forgent-pearl-miner
cat ~/.forgent/runtime/mining.json
uv run forgent mine logs --tail 200
...
## Verify ForgentAI Inference Uses Mining Endpoint

Run:

```bash
uv run forgent mine doctor
uv run forgent ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/forgent/mining/_metrics.py`.
...
```bash
uv run forgent mine stop
docker ps --filter name=forgent-pearl-miner
test ! -e ~/.forgent/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `forgent ask` no longer routes through `vllm-pearl-mining` unless another
...
- ForgentAI inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run forgent mine doctor
uv run forgent mine status || true
uv run forgent mine logs --tail 300 || true
docker inspect forgent-pearl-miner || true
