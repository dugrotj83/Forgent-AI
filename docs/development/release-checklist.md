...
2. **`forgent` (no args)** drops into a chat session within 2 s.
...
6. **`forgent doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`forgent-uninstall`** removes `~/.forgent/` and `~/.local/bin/forgent*`. Verify with `ls`.
...
- Delete `~/.forgent/config.toml` — verify bare `forgent` re-runs init.
- Delete `~/.forgent/.venv` — verify re-running curl heals it.
