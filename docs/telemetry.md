...
ForgentAI ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the ForgentAI team's PostHog instance —
...
[`src/forgent/analytics/events.py`](../src/forgent/analytics/events.py).
...
1. [`src/forgent/analytics/redaction.py`](../src/forgent/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/forgent/analytics/events.py`](../src/forgent/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the ForgentAI leaderboard. It is **not** the same as analytics and
