#!/usr/bin/env bash
# install.sh — ForgentAI curl-pipe-bash installer.
#
# Usage:
...
#   FORGENT_HOME        Install dir (default: $HOME/.forgent)
#   FORGENT_REPO_URL    git repo URL (default: https://github.com/dugrotj83/Forgent-AI.git)
#   FORGENT_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of ForgentAI can't
...
ForgentAI runs on Windows via WSL2. Two paths:
  ...
  2. Desktop app — download the .exe from the Releases page:
     https://github.com/dugrotj83/Forgent-AI/releases

See the WSL2 install guide for the full walkthrough:
  https://github.com/dugrotj83/Forgent-AI/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
ForgentAI installs to $HOME/.forgent, not /usr/local. Re-run as your
...
Two ways forward:
  ...
  2. Pre-authenticate sudo before piping (caches credentials for 5 min):
       sudo -v && curl -fsSL https://github.com/dugrotj83/Forgent-AI/install.sh | bash
EOF
        exit 1
    fi
    local sudo=""
...
# ---- env ----
# ForgentAI keeps ALL of its state (install tree + runtime data, configs,
# databases, caches, logs) under a single root so it never clutters $HOME
# beyond one directory. Relocate it by exporting FORGENT_HOME before
# running the installer, e.g.:
#     FORGENT_HOME=~/apps/forgent curl ... | bash
# The Python runtime honors the same override (and, when FORGENT_HOME is
# unset, $XDG_DATA_HOME/forgent if XDG_DATA_HOME is set). With nothing set
# the root is ~/.forgent, so existing installs are untouched.
FORGENT_HOME="${FORGENT_HOME:-$HOME/.forgent}"
FORGENT_REPO_URL="${FORGENT_REPO_URL:-https://github.com/dugrotj83/Forgent-AI.git}"
SRC_DIR="$FORGENT_HOME/src"
VENV_DIR="$FORGENT_HOME/.venv"
STATE_DIR="$FORGENT_HOME/.state"
SCRIPTS_DIR="$FORGENT_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$FORGENT_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${FORGENT_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# ForgentAI team can see where users drop off during install.
...
#
ANALYTICS_HOST="${FORGENT_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${FORGENT_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$FORGENT_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$FORGENT_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/forgent" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/forgent-wrapper.sh" "$HOME/.local/bin/forgent"
    ln -sf "$SCRIPTS_DIR/forgent-uninstall.sh" "$HOME/.local/bin/forgent-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `forgent` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "ForgentAI" "$rc" 2>/dev/null; then
...
from forgent.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "ForgentAI installer"
echo "  install dir: $FORGENT_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone ForgentAI repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install ForgentAI"    editable_install
...
#
#   PATH ok + model ok   -> "type forgent"
#   PATH new + model ok  -> "source rc && forgent  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; forgent doctor"
#   PATH new + model bad -> "source rc && forgent doctor; chat works once download finishes"
#
# `forgent` and `forgent doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="forgent"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="forgent doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'forgent doctor'
...
Background work continues silently:
  ...
  Run 'forgent doctor' to check status anytime.
EOF
