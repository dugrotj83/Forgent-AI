...
All notable changes to ForgentAI are documented in this file.
...
**Vision input for `forgent ask`** — attach images to a query with
...
install path while `forgent.ai` is down, improves desktop
first-boot diagnostics on Windows, and ships the RAM-detection fix
for Windows that missed the v1.0.1 cutoff.

### Fixed

**`forgent/traces/` missing from the v1.0.1 PyPI wheel** (#372).
...
`src/forgent/traces/` — silently dropping the whole package. Every
fresh `pip install forgent==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'forgent.traces'` on the
first `forgent ask`, learning, or server call. Anchored the pattern to
...
`forgent.ai/install.sh` URL was failing with `sslv3 alert
...
`https://github.com/dugrotj83/Forgent-AI/install.sh`, generated from
...
**`jarvis self-update`** subcommand. Detects how ForgentAI was
installed (pip, uv tool, editable git checkout) by inspecting
`forgent.__file__`, then runs the right upgrade command. Supports
...
(`FORGENT_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `FORGENT_NO_ANALYTICS`).
...
- **Editable-git users running `jarvis self-update`** get the
  ...
  checkout, not `~/ForgentAI`. If you'd come to rely on the
...
Companion blog post:
[From Minions to ForgentAI: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-forgent-retrospective).
...
`ForgentSystem` in `src/forgent/system.py`, driven by a TOML config.
...
`forgent init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/forgent/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/forgent/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/forgent/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/forgent/agents/hybrid/`.
...
A runner CLI (`python -m forgent.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.forgent/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.forgent/learning/skills/<name>/optimized.toml`
...
- **CLI commands:**
  - `forgent skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `forgent skill discover` — mine traces for recurring tool patterns
  - `forgent skill show-overlay` — inspect optimization output
  - `forgent optimize skills` — run DSPy/GEPA per-skill optimization
  - `forgent bench skills` — run the PinchBench skills benchmark
...
- `examples/forgent/spec_search_quickstart.py` — runnable end-to-end
...
- `src/forgent/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from forgent.learning.distillation.*` →
  `from forgent.learning.spec_search.*`). The `jarvis distillation`
...
- **OpenClaw category/name queries** — `forgent skill install openclaw:owner/slug` now correctly splits into category + name match.
