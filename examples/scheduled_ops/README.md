...
This tutorial demonstrates how to use ForgentAI to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Forgent` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `forgent scheduler` CLI
...
Register each script as a recurring task with `forgent scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | forgent channel send slack
...
See `forgent channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let ForgentAI auto-select from what is available.
