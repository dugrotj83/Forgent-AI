...
ForgentAI orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting ForgentAI to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- ForgentAI installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the ForgentAI channel docs).
...
ForgentAI supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/forgent/config.toml` for the full list of channel and agent
options.
...
this via the ForgentAI scheduler:

```bash
forgent scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/forgent/recipes/data/operators/` for examples.
