...
using ForgentAI. The assistant uses an orchestrator agent loop with web
...
- ForgentAI installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `forgent eval`
runner.
...
User query
  |
  v
Forgent SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `forgent agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `forgent.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/forgent/recipes/) -- composable TOML configs that
