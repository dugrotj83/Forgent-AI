...
Index a directory of documents into ForgentAI memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- ForgentAI installed (`git clone https://github.com/dugrotj83/Forgent-AI.git && cd Forgent-AI && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Forgent.memory.index()` to chunk the documents at
...
ForgentAI SDK. Adjust `--chunk-size` and `--top-k` to tune the
