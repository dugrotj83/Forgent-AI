...
A set of developer-focused scripts that use ForgentAI tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Forgent` instance with the `native_react` (ReAct) agent
...
1. **Install ForgentAI** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/forgent/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/forgent/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run forgent eval list` or
inspect `src/forgent/tools/` for the full registry.
