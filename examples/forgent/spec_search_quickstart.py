...
are local fakes, so you can ``python examples/forgent/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/forgent/examples/spec-search-quickstart.toml``
via the regular ``forgent.core.config.load_config`` machinery — you can copy
that TOML to ``~/.forgent/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    FORGENT_HOME=/tmp/forgent-spec-search-demo \\
...
from forgent.core.config import SpecSearchLearningConfig
from forgent.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from forgent.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from forgent.learning.spec_search.multi_session import SpecSearchLoop
from forgent.learning.spec_search.orchestrator import SpecSearchOrchestrator
from forgent.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from forgent.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = forgent.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.forgent/config.toml``.
    ...
    #
    # (after copying ``configs/forgent/examples/spec-search-quickstart.toml``
    # to ``~/.forgent/config.toml``)
    ...
    home = Path(
        os.environ.get("FORGENT_HOME")
        or tempfile.mkdtemp(prefix="forgent-spec-search-")
    )
    print(f"FORGENT_HOME = {home}")
    ...
    with patch(
        "forgent.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
