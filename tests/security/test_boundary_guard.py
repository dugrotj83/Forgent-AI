...
class TestBoundaryGuardScanOutbound:
    ...
    def test_redacts_openai_key(self) -> None:
        ...
        text = "Use this key: sk-prfg-abc123def456ghi789jkl012mno345pqr678stu"
        result = guard.scan_outbound(text, destination="openai")
        assert "sk-prfg-" not in result
    ...
    def test_block_mode_raises(self) -> None:
        from forgent.security.boundary import SecurityBlockError
...
class TestBoundaryGuardCheckOutbound:
    ...
    def test_redacts_tool_call_arguments(self) -> None:
        ...
        tc = ToolCall(
            ...
            name="web_search",
            arguments=(
                '{"query": "my key is sk-prfg-abc123def456ghi789jkl012mno345pqr678stu"}'
            ),
        )
        result = guard.check_outbound(tc)
        assert "sk-prfg-" not in result.arguments
...
class TestBoundaryGuardDisabled:
    ...
    def test_disabled_passes_secrets_through(self) -> None:
        ...
        text = "sk-prfg-abc123def456ghi789jkl012mno345pqr678stu"
        result = guard.scan_outbound(text, destination="openai")
        assert result == text
...
class TestEngineTagging:
    ...
    def test_litellm_engine_is_cloud(self) -> None:
        from forgent.engine.litellm import LiteLLMEngine
...
class TestToolTagging:
    ...
    def test_web_search_is_external(self) -> None:
        from forgent.tools.web_search import WebSearchTool
    ...
    def test_channel_send_is_external(self) -> None:
        from forgent.tools.channel_tools import ChannelSendTool
...
class TestToolExecutorBoundaryIntegration:
    ...
    def test_external_tool_args_scanned(self) -> None:
        ...
        tc = ToolCall(
            ...
            name="fake_external",
            arguments=(
                '{"q": "my key is sk-prfg-abc123def456ghi789jkl012mno345pqr678stu"}'
            ),
        )
        result = executor.execute(tc)
        assert "sk-prfg-" not in result.content
    ...
    def test_no_guard_passes_through(self) -> None:
        ...
        tc = ToolCall(
            id="t2",
            name="fake_external",
            arguments='{"q": "sk-prfg-abc123def456ghi789jkl012mno345pqr678stu"}',
        )
        result = executor.execute(tc)
        assert "sk-prfg-" in result.content
