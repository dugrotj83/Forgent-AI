"""Regression tests for forgent.mcp.loader.load_mcp_tools_from_config.

Closes the gap that #461 surfaced — MCP tools were silently dropped on
`forgent ask` and `forgent serve` because neither path read
...
def _mock_mcp_stack():
    ...
    with (
        patch("forgent.mcp.client.MCPClient") as MockClient,
        patch("forgent.mcp.transport.StreamableHTTPTransport") as MockHttp,
        patch("forgent.mcp.transport.StdioTransport") as MockStdio,
        patch("forgent.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
