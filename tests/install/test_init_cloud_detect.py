"""Tests for init's cloud auto-detect and from-bare-forgent flag."""
...
def test_init_accepts_from_bare_forgent_flag(
    tmp_forgent_home: Path, monkeypatch
) -> None:
    """The --from-bare-forgent flag exists and suppresses the launch-chat prompt."""
    ...
    result = runner.invoke(
        init,
        ["--from-bare-forgent", "--engine", "ollama", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
    # When --from-bare-forgent, we expect NOT to see a "launch chat" affordance.
...
def test_init_from_bare_forgent_skips_engine_prompt(
    tmp_forgent_home: Path, monkeypatch
) -> None:
    """--from-bare-forgent must not hang on the engine-selection prompt
    ...
    result = runner.invoke(
        init,
        ["--from-bare-forgent", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
