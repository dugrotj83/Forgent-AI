"""Tests for the env-aware ForgentAI home-directory resolver (issue #462).

Covers the single-root consolidation: ``$FORGENT_HOME`` >
``$XDG_DATA_HOME/forgent`` > ``~/.forgent``, backward compatibility
(no env => exactly ``~/.forgent``), and the source-tree rejection guard.
"""
...
from forgent.core import paths
...
class TestGetConfigDir:
    ...
    def test_default_when_unset_is_legacy_dir(
        ...
        # historical ~/.forgent so existing installs are untouched.
        _clear_env(monkeypatch)
        assert paths.get_config_dir() == (Path.home() / ".forgent").resolve()

    def test_respects_forgent_home(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _clear_env(monkeypatch)
        custom = tmp_path / "oj"
        monkeypatch.setenv("FORGENT_HOME", str(custom))
    ...
    def test_respects_xdg_data_home(
        ...
        # Single nested 'forgent' dir under XDG_DATA_HOME.
        assert paths.get_config_dir() == (tmp_path / "forgent").resolve()

    def test_forgent_home_wins_over_xdg(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _clear_env(monkeypatch)
        oj = tmp_path / "oj_wins"
        monkeypatch.setenv("FORGENT_HOME", str(oj))
        monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg_loses"))
        assert paths.get_config_dir() == oj.resolve()

    def test_expands_user_in_forgent_home(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _clear_env(monkeypatch)
        monkeypatch.setenv("FORGENT_HOME", "~/relocated-oj")
    ...
    def test_returns_absolute_path(
        ...
        _clear_env(monkeypatch)
        monkeypatch.setenv("FORGENT_HOME", str(tmp_path / "rel"))
...
class TestDerivedDirs:
    ...
    def test_config_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        _clear_env(monkeypatch)
        monkeypatch.setenv("FORGENT_HOME", str(tmp_path / "oj"))
    ...
    def test_cache_dir_under_xdg(
        ...
        _clear_env(monkeypatch)
        monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
        assert paths.get_cache_dir() == (tmp_path / "forgent" / "cache").resolve()
...
class TestSourceTreeRejection:
    ...
    def test_rejects_path_inside_source_tree(
        ...
        monkeypatch.setenv("FORGENT_HOME", str(source_root / "junk_dir"))
...
class TestLegacyConstantsHonorEnv:
    ...
    FORGENT_HOME entirely. The constant is resolved once at import (the
    ...
    def test_constant_matches_resolver_at_import(self) -> None:
        from forgent.core import config
    ...
    def test_dataclass_defaults_reflect_env(
        ...
        # instantiation time, not freeze ~/.forgent at import.
        _clear_env(monkeypatch)
        from forgent.core.config import SessionConfig, StorageConfig
    ...
    def test_downstream_consumer_honors_env(
        ...
        _clear_env(monkeypatch)
        from forgent.core import credentials
