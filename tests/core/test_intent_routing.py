"""Tests for intent-based agent routing in ForgentSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal ForgentSystem instance for testing _detect_agent_intent."""
        from forgent.system import ForgentSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = ForgentSystem.__new__(ForgentSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("forgent.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_forgent_triggers_digest(self, system):
        with patch("forgent.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Forgent")
