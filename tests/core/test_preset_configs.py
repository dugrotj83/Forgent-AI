...
Presets are installed via `forgent init --preset <name>`, which copies
`configs/forgent/examples/<name>.toml` to `~/.forgent/config.toml`.
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `forgent init` expects to be populated for a working first run.
