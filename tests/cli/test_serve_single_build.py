"""Regression tests for #263 — ``forgent serve`` must build the system once.
...
1. ``SystemBuilder.build`` is never called during ``forgent serve`` startup
...
# ``forgent.cli.serve`` as an attribute resolves to the click *command*
# (re-exported on the package); grab the real module to monkeypatch its globals.
serve_mod = importlib.import_module("forgent.cli.serve")
...
def _repopulate_registries() -> None:
    ...
    if not AgentRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("forgent.agents.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
...
def _run_serve(tmp_path, monkeypatch, *, build_spy, set_system_spy):
    """Invoke ``forgent serve`` with all heavy/blocking pieces stubbed out.
    ...
    monkeypatch.setattr("forgent.security.setup_security", lambda *a, **k: sec)

    with (
        patch(
            "forgent.system.builder.SystemBuilder.build",
            build_spy,
        ),
        patch(
            "forgent.agents.executor.AgentExecutor.set_system",
            set_system_spy,
        ),
...
def test_serve_does_not_call_systembuilder_build(tmp_path, monkeypatch):
    ...
    build_spy = MagicMock(
        side_effect=AssertionError(
            "SystemBuilder.build() must not run during `forgent serve` startup "
