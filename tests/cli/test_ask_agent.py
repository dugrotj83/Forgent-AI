"""Tests for ``forgent ask --agent`` CLI integration."""
...
from forgent.agents._stubs import AgentContext, AgentResult, ToolUsingAgent
from forgent.cli import cli
from forgent.core.types import ToolCall, ToolResult
from forgent.tools._stubs import BaseTool, ToolSpec

_ask_mod = importlib.import_module("forgent.cli.ask")
...
def agent_setup():
    ...
    from forgent.core.registry import AgentRegistry, ToolRegistry
...
def mock_setup():
    ...
    with (
        ...
        mock_cfg.return_value = ForgentConfig()
...
class TestAskAgentOption:
    ...
    def test_no_agent_flag_falls_back_to_config_default_agent(self, runner, mock_setup):
        ...
        The default ``ForgentConfig`` sets ``default_agent = "simple"``, so
        ``forgent ask "..."`` should route through SimpleAgent rather than
...
class TestPersonaFilesReachModel:
    ...
    def test_soul_md_content_reaches_engine_in_simple_agent(
        ...
        soul.write_text("PERSONA_SENTINEL_zh_forgent", encoding="utf-8")
        ...
        assert "PERSONA_SENTINEL_zh_forgent" in joined
