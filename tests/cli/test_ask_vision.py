"""CLI-level regression tests for ``forgent ask`` vision input.
...
from forgent.core.types import Role
...
def _patch_ask(monkeypatch, tmp_path: Path, *, engine_name: str) -> _RecordingEngine:
    """Wire ``forgent ask`` to a recording engine reported under ``engine_name``."""
    cfg = ForgentConfig()
