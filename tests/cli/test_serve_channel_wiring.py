"""Tests for ForgentSystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on ForgentSystem directly. The serve.py
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> ForgentSystem:
    """Build a minimal ForgentSystem with mock engine for testing wire_channel."""
    config = ForgentConfig()
    if tmp_path is not None:
        config.sessions.db_path = str(tmp_path / "sessions.db")
    mock_engine = engine or MagicMock()
    return ForgentSystem(
        config=config,
...
# ---------------------------------------------------------------------------
# Tests via ForgentSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """ForgentSystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """ForgentSystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """ForgentSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = ForgentConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
