"""Tests for ``forgent chat`` interactive REPL command."""
...
from forgent.cli.chat_cmd import _read_input, chat
from forgent.core.config import ForgentConfig
from forgent.core.events import Event, EventBus, EventType
...
class TestChatAgents:
    def test_simple_agent_does_not_receive_tool_only_kwargs(self) -> None:
        ...
        with (
            patch("forgent.cli.chat_cmd.load_config", return_value=config),
            patch("forgent.engine.get_engine", return_value=("mock", engine)),
            patch("forgent.intelligence.register_builtin_models"),
        ):
    ...
    def test_memory_service_started_fed_and_stopped(self) -> None:
        ...
        with (
            ...
            patch("forgent.intelligence.register_builtin_models"),
            patch(
                "forgent.memory.build_memory_service",
