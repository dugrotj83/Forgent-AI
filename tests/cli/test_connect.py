"""Tests for ``forgent connect`` CLI command."""
...
def test_connect_list_no_connectors() -> None:
    ...
    runner = CliRunner()
    with mock.patch(
        "forgent.cli.connect_cmd.connect.__wrapped__"
        if hasattr(cli, "__wrapped__")
        else "forgent.core.registry.ConnectorRegistry.items",
        return_value=(),
    ):
        with mock.patch(
            "forgent.core.registry.ConnectorRegistry.items",
            return_value=(),
        ):
...
def test_connect_specific_source(tmp_path: object) -> None:
    ...
    with (
        mock.patch(
            "forgent.core.registry.ConnectorRegistry.contains",
            return_value=True,
        ),
        mock.patch(
            "forgent.core.registry.ConnectorRegistry.get",
