...
from forgent.cli import _version_check
from forgent.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("FORGENT_NO_UPDATE_CHECK", "CI", "FORGENT_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.forgent state.
    monkeypatch.setenv("FORGENT_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_forgent_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("FORGENT_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("FORGENT_CONFIG", str(cfg))
    ...
    def test_forgent_config_env_override(self, monkeypatch, tmp_path):
        """FORGENT_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("forgent.cli._version_check._do_check")
    ...
    @patch(
        "forgent.cli._version_check._do_check",
