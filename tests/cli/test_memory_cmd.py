"""Tests for ``forgent memory`` CLI commands."""
...
from forgent.memory.store import LocalFactStore
...
def test_memory_index_file(tmp_path: Path, monkeypatch):
    ...
    mod = importlib.import_module("forgent.cli.memory_cmd")
    monkeypatch.setattr(
        mod,
        "_get_backend",
...
def _patch_fact_store(monkeypatch, tmp_path: Path) -> LocalFactStore:
    """Point ``forgent memory list/clear`` at a temp fact store."""
    mod = importlib.import_module("forgent.cli.memory_cmd")
