...
from forgent.server.api_routes import include_all_routes  # noqa: E402
...
class TestMemoryRustMissing:
    """Regression for #502: when the native ``forgent_rust`` extension is
    ...
    def _client(monkeypatch):
        ...
        def _boom():
            raise ImportError("No module named 'forgent_rust'")

        import forgent._rust_bridge as bridge
    ...
    def test_config_reports_unavailable(self, monkeypatch):
        ...
        assert "forgent_rust" in (data["detail"] or "")
...
class TestMetricsRoute:
    def test_metrics_endpoint(self):
        ...
        assert resp.status_code == 200
        assert "forgent" in resp.text or "No metrics" in resp.text
