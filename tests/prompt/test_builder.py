...
def test_build_frozen_prefix(memory_dir: Path):
    ...
    builder = SystemPromptBuilder(
        agent_template="You are Forgent.",
...
def test_config_prefix_prepended(memory_dir: Path):
    ...
    builder = SystemPromptBuilder(
        ...
        system_prompt_config=SystemPromptConfig(prefix="ALWAYS ANSWER AS FORGENT."),
    )
    prompt = builder.build()
    assert prompt.startswith("ALWAYS ANSWER AS FORGENT.")
    assert "You are Forgent." in prompt
