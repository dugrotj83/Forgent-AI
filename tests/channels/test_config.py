...
from forgent.core.config import ChannelConfig, ForgentConfig, load_config
...
class TestChannelConfigInForgentConfig:
    def test_channel_config_in_forgent_config(self) -> None:
        cfg = ForgentConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_forgent_config_channel_defaults(self) -> None:
        cfg = ForgentConfig()
