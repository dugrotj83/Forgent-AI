...
``~/.forgent/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    *source* (``forgent.core.config``) — every connector re-derives
    ...
    import forgent.connectors.oauth as oauth_mod
    import forgent.core.config as config_mod
    import forgent.server.connectors_router as router_mod
    ...
    google_mods = [
        "forgent.connectors.gdrive",
        "forgent.connectors.gcalendar",
        "forgent.connectors.gcontacts",
        "forgent.connectors.gmail",
        "forgent.connectors.google_tasks",
    ]
    ...
    # subsequent tests in the same process see ~/.forgent again.
    for name in google_mods:
        if name in sys.modules:
            importlib.reload(sys.modules[name])
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("forgent.core.open_browser") as mock_browser:
        resp = client.post(
...
def test_oauth_callback_exchanges_and_connects(
    ...
    from forgent.connectors.gdrive import GDriveConnector
