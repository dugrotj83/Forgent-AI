"""Verify ForgentDirectBackend.generate_full returns the spec §6.2 extended fields."""
...
class TestForgentDirectExtendedFields:
    def test_generate_full_includes_framework_and_commit(self) -> None:
        ...
        # engine); set required attrs directly. ForgentDirectBackend.generate_full
        # calls ``self._system.engine.generate(messages, ...)`` so we mock the
        # whole ``_system`` chain.
        backend = ForgentDirectBackend.__new__(ForgentDirectBackend)
        ...
        assert result["framework"] == "forgent"
