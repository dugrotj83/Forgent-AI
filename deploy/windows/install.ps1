<#
.SYNOPSIS
    ForgentAI native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the ForgentAI repository to $env:LOCALAPPDATA\ForgentAI
         (override with $env:FORGENT_HOME).
      ...
      7. Optionally register the scheduled-task service (see
         deploy/windows/forgent-service.ps1).

    Usage (one-liner):
      irm https://github.com/dugrotj83/Forgent-AI/install.ps1 | iex

    Usage (file invocation, supports flags):
      irm https://github.com/dugrotj83/Forgent-AI/install.ps1 -OutFile install.ps1
    ...
    are honored via env vars when the corresponding flag is absent:
      $env:FORGENT_SKIP_SERVICE = '1'
      $env:FORGENT_SERVICE      = '1'
      $env:FORGENT_FORCE        = '1'
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:FORGENT_API_KEY (an
...
if (-not $SkipService -and $env:FORGENT_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:FORGENT_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:FORGENT_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but ForgentAI requires
...
$installRoot = if ($env:FORGENT_HOME) {
    $env:FORGENT_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'ForgentAI'
}
...
$repoUrl = if ($env:FORGENT_REPO_URL) {
    $env:FORGENT_REPO_URL
} else {
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'forgent' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. forgent.cmd shim - so bare `forgent` works in any new PowerShell
# ---------------------------------------------------------------------------

$binDir = Join-Path $installRoot 'bin'
$shimPath = Join-Path $binDir 'forgent.cmd'
...
# so the shim is self-locating - moving %LOCALAPPDATA%\ForgentAI won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" forgent %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\ForgentAI\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\ForgentAI\bin` (unexpanded) into User
...
Write-Ok "forgent shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\forgent-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register ForgentAI as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'ForgentAI' registered (loopback default)."
}
...
Write-Host "  |   ForgentAI install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'forgent' } else { 'forgent doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the forgent shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'forgent doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN ForgentAI     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://github.com/dugrotj83/Forgent-AI/"
Write-Host ""
