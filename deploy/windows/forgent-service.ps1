<#
.SYNOPSIS
    Register / unregister the ForgentAI Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/forgent.service and
    deploy/launchd/com.forgent.plist.

    Registers a per-user scheduled task named "ForgentAI" that starts
    `forgent serve` at logon and restarts on failure. Loopback default
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\ForgentAI (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:FORGENT_API_KEY
                           — the server refuses to start unauthenticated
                           on a non-loopback bind.
      -ListenPort <int>    default: 8000

    Usage:
      powershell -ExecutionPolicy Bypass -File forgent-service.ps1 install
      powershell -ExecutionPolicy Bypass -File forgent-service.ps1 uninstall
      powershell -ExecutionPolicy Bypass -File forgent-service.ps1 status
#>
...
function Get-DefaultInstallRoot {
    ...
    if ($env:FORGENT_HOME) { return $env:FORGENT_HOME }
    return (Join-Path $env:LOCALAPPDATA 'ForgentAI')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "ForgentAI source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/forgent.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:FORGENT_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:FORGENT_API_KEY is
not set. An unauthenticated non-loopback bind is refused by forgent serve
and would also create a security hole. Set the env var first:

    `$env:FORGENT_API_KEY = (uv run forgent auth generate-key)
...
"@
    ...
    # `forgent serve` would refuse to bind 0.0.0.0 — failing silently every
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting FORGENT_API_KEY to User environment so the scheduled task can read it at logon."
    ...
    $action = New-ScheduledTaskAction `
        -Execute $uvPath `
        -Argument "run forgent serve --host $ListenHost --port $ListenPort" `
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'ForgentAI API server (loopback default — see deploy/windows/README.md)' | Out-Null
