...
5. Clones the ForgentAI repository to `%LOCALAPPDATA%\ForgentAI`
...
The available env vars: `FORGENT_SKIP_SERVICE`, `FORGENT_SERVICE`,
`FORGENT_FORCE`. If you need richer control, save the script first
...
remove the task with `forgent-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\ForgentAI\src\deploy\windows\forgent-service.ps1"
...
`forgent-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:FORGENT_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/forgent/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/forgent.service` | `deploy/launchd/com.forgent.plist` | `deploy/windows/forgent-service.ps1` (cmdlet-driven) |
...
```powershell
cd "$env:LOCALAPPDATA\ForgentAI\src"
git pull --ff-only
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\ForgentAI\src\deploy\windows\forgent-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\ForgentAI"
```
