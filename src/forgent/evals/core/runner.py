...
from forgent.evals.core.backend import InferenceBackend
from forgent.evals.core.dataset import DatasetProvider
from forgent.evals.core.export import _hardware_info_dict
from forgent.evals.core.scorer import Scorer
from forgent.evals.core.tracker import ResultTracker
...
try:
    from forgent.telemetry.efficiency import compute_efficiency
except ImportError:  # pragma: no cover
    compute_efficiency = None  # type: ignore[assignment]

try:
    from forgent.telemetry.efficiency import estimate_model_flops_per_token
except ImportError:  # pragma: no cover
...
class EvalRunner:
    ...
    def run(
        ...
        from forgent.evals.core.dataset import DatasetProvider as _DP
    ...
    def _process_one(self, record: EvalRecord) -> EvalResult:
        ...
        def _backend_error_result(full: dict, message: str) -> EvalResult:
            ...
            return EvalResult(
                ...
                framework=full.get(
                    "framework",
                    getattr(self._backend, "framework_name", "forgent"),
                ),
        ...
        try:
            ...
            return EvalResult(
                ...
                framework=full.get("framework", "forgent"),
            ...
            return EvalResult(
                ...
                model_answer="",
                error=str(exc),
                framework=getattr(self._backend, "framework_name", "forgent"),
