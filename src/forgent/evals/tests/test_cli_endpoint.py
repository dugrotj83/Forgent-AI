...
dropped for forgent-direct/forgent-agent and ignored by terminalbench-native
...
from forgent.evals.cli import _build_backend, _run_terminalbench_native
...
class TestBuildBackendForwardsEndpoint:
    @patch("forgent.evals.backends.forgent_direct.ForgentDirectBackend")
    def test_forgent_direct_receives_base_url_and_api_key(self, mock_cls):
        _build_backend(
            "forgent-direct",
            "vllm",
            "orchestrator",
    ...
    @patch("forgent.evals.backends.forgent_agent.ForgentAgentBackend")
    def test_forgent_agent_receives_base_url_and_api_key(self, mock_cls):
        _build_backend(
            "forgent-agent",
            "vllm",
            "orchestrator",
...
class TestTerminalBenchNativeApiBase:
    @patch("forgent.evals.backends.terminalbench_native.TerminalBenchNativeBackend")
...
class TestRunSingleSuiteModeGating:
    @patch("forgent.evals.cli._run_terminalbench_native")
