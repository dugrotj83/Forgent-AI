...
class TestForgentDirectBackend:
    @patch("forgent.system.SystemBuilder")
    def test_construction_default(self, mock_builder_cls):
        ...
        backend = ForgentDirectBackend()
        assert backend.backend_id == "forgent-direct"
    ...
    def test_construction_with_engine_key(self, mock_builder_cls):
        ...
        ForgentDirectBackend(engine_key="cloud")
...
class TestForgentAgentBackend:
    ...
    def test_construction(self, mock_builder_cls):
        ...
        backend = ForgentAgentBackend(
            engine_key="cloud",
            agent_name="orchestrator",
            tools=["calculator", "think"],
        )
        assert backend.backend_id == "forgent-agent"
        mock_builder.engine.assert_called_with("cloud")
    ...
    def test_generate_full(self, mock_builder_cls):
        ...
        backend = ForgentAgentBackend(agent_name="orchestrator")
...
class TestForgentDirectBackendBaseUrl:
    """--base-url targeting for the forgent-direct backend."""

    @patch("forgent.system.SystemBuilder")
    def test_base_url_injects_pinned_openai_compat_engine(self, mock_builder_cls):
        from forgent.engine.openai_compat_engines import OpenAICompatEngine
        from forgent.evals.backends.forgent_direct import ForgentDirectBackend
        ...
        with respx.mock:
            ...
            ForgentDirectBackend(base_url="http://127.0.0.1:18999/v1", api_key="sk-x")
    ...
    def test_unreachable_base_url_fails_fast_naming_url(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            with pytest.raises(RuntimeError, match=r"http://127\.0\.0\.1:18998"):
                ForgentDirectBackend(base_url="http://127.0.0.1:18998")
    ...
    def test_no_base_url_keeps_engine_key_path(self, mock_builder_cls):
        ...
        ForgentDirectBackend(engine_key="vllm")
    ...
    def test_base_url_wins_over_engine_key(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            ForgentDirectBackend(engine_key="vllm", base_url="http://127.0.0.1:18999")
...
class TestForgentAgentBackendBaseUrl:
    """--base-url targeting for the forgent-agent backend."""
    ...
    def test_base_url_injects_pinned_openai_compat_engine(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            ForgentAgentBackend(base_url="http://127.0.0.1:18999/v1", api_key="sk-x")
    ...
    def test_unreachable_base_url_fails_fast_naming_url(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            with pytest.raises(RuntimeError, match=r"http://127\.0\.0\.1:18998"):
                ForgentAgentBackend(base_url="http://127.0.0.1:18998")
    ...
    def test_no_base_url_keeps_engine_key_path(self, mock_builder_cls):
        ...
        ForgentAgentBackend(engine_key="vllm")
        mock_builder.engine.assert_called_with("vllm")
        mock_builder.engine_instance.assert_not_called()
