"""Forgent Agent backend — agent-level inference with tool calling."""
...
from forgent.evals.backends._commit_util import forgent_commit
from forgent.evals.core.backend import InferenceBackend


class ForgentAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + ForgentSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "forgent-agent"
    framework_name = "forgent"
    ...
    def __init__(
        ...
        if base_url:
            ...
            from forgent.evals.backends._endpoint_util import (
        ...
        # Override the agent's per-run turn budget. ForgentConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """ForgentAI repo HEAD commit (for telemetry tagging)."""
        from forgent.evals.backends._commit_util import forgent_commit

        return forgent_commit()
    ...
    def generate_full(
        ...
        *,
        ...
        return {
            ...
            "framework": "forgent",
            "framework_commit": forgent_commit(),
            "error": None,
        }
