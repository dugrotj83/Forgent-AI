...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import forgent.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from forgent.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from forgent.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from forgent.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from forgent.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    cannot be built because the mandatory ``forgent_rust`` extension is not
    ...
    if backend is None:
        from forgent.tools.storage._stubs import MemoryBackendUnavailable

        try:
            from forgent.tools.storage.sqlite import SQLiteMemory
...
async def memory_config(request: Request):
    ...
    ``backend_type: sqlite``) when the native ``forgent_rust`` extension is
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from forgent.security.file_policy import is_sensitive_file
        from forgent.tools.storage.ingest import ingest_path

        target = Path(req.path).expanduser().resolve()
        if not target.exists():
            raise HTTPException(status_code=404, detail=f"Path not found: {req.path}")

        # Sandbox: when workspace roots are configured via FORGENT_WORKSPACE
        ...
        workspace = os.environ.get("FORGENT_WORKSPACE", "").strip()
        if workspace:
            roots = [
...
async def telemetry_stats(request: Request):
    ...
    try:
        ...
        from forgent.telemetry.aggregator import TelemetryAggregator
...
async def list_skills(request: Request):
    ...
    try:
        from forgent.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.forgent/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from forgent.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP forgent_requests_total Total requests processed",
            "# TYPE forgent_requests_total counter",
            f"forgent_requests_total {stats.get('total_requests', 0)}",
            "# HELP forgent_tokens_total Total tokens generated",
            "# TYPE forgent_tokens_total counter",
            f"forgent_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP forgent_latency_avg_ms Average latency in milliseconds",
            "# TYPE forgent_latency_avg_ms gauge",
            f"forgent_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def _record_ws_trace(
    ...
    *,
    ...
    if trace_store is None or not result:
        return
    from forgent.traces.collector import record_response_trace
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from forgent.server.auth_middleware import websocket_authorized
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from forgent.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from forgent.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from forgent.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from forgent.server.ws_bridge import create_ws_router
