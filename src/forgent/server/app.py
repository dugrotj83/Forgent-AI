"""FastAPI application factory for the ForgentAI API server."""
...
from forgent.server.analytics_routes import router as analytics_router
from forgent.server.api_routes import include_all_routes
from forgent.server.comparison import comparison_router
from forgent.server.connectors_router import create_connectors_router
from forgent.server.dashboard import dashboard_router
from forgent.server.digest_routes import create_digest_router
from forgent.server.research_router import router as research_router
from forgent.server.routes import router
from forgent.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from forgent.server.channel_bridge import ChannelBridge
                    from forgent.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional ForgentConfig for other settings.
    """
    app = FastAPI(
        title="ForgentAI API",
        description="OpenAI-compatible API server for ForgentAI",
        version="0.1.0",
    )
    ...
    try:
        from forgent.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from forgent.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from forgent.server.webhook_routes import (
