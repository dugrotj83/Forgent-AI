...
from forgent.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a ForgentSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after ForgentSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from forgent.agents import AgentRegistry
        ...
        # Resolve engine + model from ForgentSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in ForgentSystem")
        model = (
            config.get("model")
        ...
        # agent's own ToolExecutor. Set by CLI paths like `forgent agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `forgent ask` path so they no
    ...
    def _save_trace(
        ...
        from forgent.core.types import StepType, Trace, TraceStep
