...
Why not just use ForgentAI's :class:`InferenceEngine` for both? Two reasons:
...
from forgent.agents._stubs import AgentContext, AgentResult, BaseAgent
from forgent.agents.hybrid._openai_retry import patch_openai_globally
from forgent.agents.hybrid._prices import (
    NO_TEMP_PREFIXES,
    is_gpt5_family,
...
def tavily_search_context(
    ...
    *,
    ...
    """Run ForgentAI WebSearchTool and return accounting-friendly metadata."""
...
#   override via ``FG_OPENROUTER_MAX_CONCURRENT``. Held only around the SDK
#   ``client.chat.completions.create`` invocation, not the bookkeeping.
# - **RPM** — a sliding window of timestamps in a ``deque``. Default 60
#   requests/minute; override via ``FG_OPENROUTER_RPM``. If the deque has
...
def _openrouter_limiter() -> _OpenRouterLimiter:
    ...
    if _OPENROUTER_LIMITER is None:
        with _OPENROUTER_LIMITER_LOCK:
            if _OPENROUTER_LIMITER is None:
                max_concurrent = int(
                    os.environ.get("FG_OPENROUTER_MAX_CONCURRENT", "20") or 20
                )
                rpm = int(os.environ.get("FG_OPENROUTER_RPM", "60") or 60)
...
class LocalCloudAgent(BaseAgent):
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the ForgentAI-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/forgent/engine/cloud.py``) — we strip it here.
