...
both forced by the ForgentAI environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. ForgentAI
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — ForgentAI scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    ForgentAI substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
