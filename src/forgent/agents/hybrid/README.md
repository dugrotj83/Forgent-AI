...
harness — each is registered as a standard ForgentAI agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$FORGENT_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.forgent/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These ForgentAI ports
...
`src/forgent/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m forgent.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$FORGENT_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.forgent-hybrid/experiments/`). The schema matches the
...
```bash
src/forgent/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.forgent/experiments/hybrid/docs/results.md`) at full N —
...
these ForgentAI cells should reproduce the harness numbers within
