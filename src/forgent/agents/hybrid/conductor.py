...
from forgent.agents.hybrid._base import (
...
from forgent.agents.hybrid.mini_swe_agent import (
    _clone_repo,
    _extract_diff,
    run_swe_agent_loop,
)
...
def _default_pool(
    ...
    Each provider is gated by an ``FG_CONDUCTOR_DISABLE_*`` env var so a cell
    ...
    if not os.environ.get("FG_CONDUCTOR_DISABLE_GEMINI"):
        pool.append(
            {
                "id": len(pool),
                "name": "gemini-pro",
    ...
    if not os.environ.get("FG_CONDUCTOR_DISABLE_ANTHROPIC"):
        pool.append(
            {
                "id": len(pool),
    ...
    if not os.environ.get("FG_CONDUCTOR_DISABLE_OPENAI"):
        pool.append(
            {
                "id": len(pool),
                "name": "gpt-5",
    ...
    if not os.environ.get("FG_CONDUCTOR_DISABLE_OPENROUTER"):
        pool.append(
            {
                "id": len(pool),
