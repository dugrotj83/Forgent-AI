...
::
    ...
    python -m forgent.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3

Reads a cell definition from ``registry/<method>.toml`` (bundled with this
package or pointed at by ``FORGENT_HYBRID_REGISTRY_DIR``), constructs
the registered agent, loads bench tasks via ForgentAI's existing dataset
...
existing rescore / dashboard scripts can read ForgentAI cells without
modification.
"""
...
from forgent.agents.hybrid._energy import EnergyCollector
from forgent.agents.hybrid._prompts import format_prompt as _format_prompt
...
DEFAULT_EXPERIMENTS_DIR = Path(
    os.environ.get(
        "FORGENT_HYBRID_EXPERIMENTS_DIR",
...
# ``FORGENT_HYBRID_TASK_TIMEOUT_S`` (0 / negative disables).
DEFAULT_TASK_TIMEOUT_S = float(
    os.environ.get("FORGENT_HYBRID_TASK_TIMEOUT_S", "1800") or 1800
)
...
def load_registry(registry_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    env_override = os.environ.get("FORGENT_HYBRID_REGISTRY_DIR")
    if env_override:
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    for rec in ds.iter_records():
        ...
        # ForgentAI prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _get_gaia_scorer():
    ...
    ``FORGENT_GAIA_JUDGE_MODEL``); the judge backend is the ``cloud``
    engine, so ``FORGENT_CONFIG`` needs a ``[engine.cloud]`` section.
    """
    global _GAIA_SCORER
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                from forgent.evals.backends.forgent_direct import (
                    ForgentDirectBackend,
                )
                from forgent.evals.scorers.gaia_exact import GAIAScorer

                judge_model = os.environ.get(
                    "FORGENT_GAIA_JUDGE_MODEL", "gpt-5-mini-2025-08-07"
                )
                try:
                    backend = ForgentDirectBackend(engine_key="cloud")
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    Uses the shared ForgentAI :class:`GAIAScorer`. The previous version
...
def _score_swebench(
    ...
    """
    from forgent.evals.core.types import EvalRecord
    from forgent.evals.scorers.swebench_harness import (
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import forgent.agents  # noqa: F401 — populate registry
...
def _run_cell_locked(
    ...
    *,
    ...
    # (env ``FORGENT_HYBRID_TASK_TIMEOUT_S``, 1800s) applies. 0 disables.
    mcfg = cell.get("method_cfg") or {}
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m forgent.agents.hybrid.runner",
