"""Central, env-aware resolution of ForgentAI' home directory.

ForgentAI keeps all of its runtime state (config, databases, caches, logs,
...
1. ``$FORGENT_HOME`` — explicit override (also honored by the shell
   installer, see ``scripts/install/install.sh``).
2. ``$XDG_DATA_HOME/forgent`` — when ``$XDG_DATA_HOME`` is set, follow the
   XDG Base Directory spec by nesting a single ``forgent`` directory under
...
3. ``~/.forgent`` — the historical default. With no env vars set, the
...
override. Modules that previously hardcoded ``Path.home() / ".forgent"``
should call :func:`get_config_dir` (or :func:`get_data_dir` /
:func:`get_cache_dir`) instead.

Defense in depth: the resolved root must never live inside the ForgentAI
source tree (a misconfigured ``$FORGENT_HOME`` pointing at the repo would
...
_DEFAULT_DIR_NAME = ".forgent"
_XDG_SUBDIR_NAME = "forgent"
...
def _find_source_root() -> Path | None:
    """Walk upward from this module to find the ForgentAI source root.

    Returns the directory containing the ForgentAI ``pyproject.toml`` (the one
    whose ``name = "forgent"``), or ``None`` when running from an installed
    ...
    for candidate in (here, *here.parents):
        ...
        if py.exists():
            try:
                ...
                continue
            if 'name = "forgent"' in content.lower():
                return candidate
    return None


def _reject_source_tree(path: Path) -> Path:
    """Raise if ``path`` resolves inside the ForgentAI source tree."""
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"ForgentAI home ({path}) is inside the source tree "
                f"({source_root}). ForgentAI refuses to write runtime state "
                "inside its own repo. Set FORGENT_HOME (or XDG_DATA_HOME) "
                "to a directory outside the repo (default: ~/.forgent)."
            )
    return path


def get_config_dir() -> Path:
    """Resolve ForgentAI' single root directory, honoring env overrides.

    Precedence: ``$FORGENT_HOME`` > ``$XDG_DATA_HOME/forgent`` >
    ``~/.forgent``. The result is always absolute and is rejected if it
    falls inside the ForgentAI source tree.
    """
    env_home = os.environ.get("FORGENT_HOME")
    if env_home:
...
def get_config_path() -> Path:
    """Resolve the path to ``config.toml`` under the ForgentAI root."""
...
def get_cache_dir() -> Path:
    ...
    Lives at ``<root>/cache`` so caches stay inside the single ForgentAI
