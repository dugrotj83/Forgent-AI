...
User configuration lives at ``~/.forgent/config.toml``.  ``load_config()``
...
from forgent.core.paths import (
    ConfigurationError,
    get_cache_dir,
    get_config_dir,
    get_config_path,
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``ForgentConfig.mining``
...
# once at import via the env-aware resolver in ``forgent.core.paths`` (the
# install-script model: ``FORGENT_HOME`` / ``XDG_DATA_HOME`` are set before
...
def _ensure_config_dir() -> Path:
    ...
    from forgent.security.file_utils import secure_mkdir
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from forgent.intelligence.model_catalog import BUILTIN_MODELS
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e forgent[learning-ace]`` once the
    ...
    # most ForgentAI users have credentials for.
    ...
    # Empty string defaults to ``~/.forgent/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "forgent"
...
class StorageConfig:
    ...
    # the background and persists them across sessions (see forgent.memory).
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        "You are ForgentAI, a helpful AI assistant running locally on the "
        ...
        "who or what you are, identify yourself as ForgentAI. Respond "
...
class AnalyticsConfig:
    ...
    usage events sent to the ForgentAI team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.forgent/whatsapp_auth
    assistant_name: str = "Forgent"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "forgent-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.forgent/scheduler.db
...
class SkillsConfig:
    ...
    index_repo: str = "https://github.com/forgent/skill-index.git"
    index_dir: str = field(
...
@dataclass
class ForgentConfig:
    """Top-level configuration for ForgentAI."""
...
# Sections that users may set via ``forgent config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(ForgentConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``ForgentConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import forgent.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "ForgentConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> ForgentConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``FORGENT_CONFIG`` when set,
        otherwise ``~/.forgent/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = ForgentConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("FORGENT_CONFIG"):
        config_path = Path(os.environ["FORGENT_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``forgent init``)
# ---------------------------------------------------------------------------
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.forgent/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# background and persists them. Starts/stops with `forgent serve` and
# `forgent chat`; manage stored facts with `forgent memory list` / `clear`.
[memory]
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.forgent/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.forgent/whatsapp_auth
