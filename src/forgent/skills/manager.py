...
from forgent.skills.dependency import validate_dependencies
from forgent.skills.executor import SkillExecutor, SkillResult
from forgent.skills.loader import discover_skills
from forgent.skills.tool_adapter import SkillTool
from forgent.skills.types import SkillManifest
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.forgent/learning/skills/ if config can't be loaded.
            try:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.forgent.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from forgent.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("forgent", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["forgent"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.forgent.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("forgent", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.forgent/skills/discovered/``).  Returns a list of dicts with
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "forgent (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from forgent.skills.loader import load_skill_directory
