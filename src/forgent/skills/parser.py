...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.forgent.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "forgent_meta" the value is
# stored under manifest.metadata["forgent"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("forgent_meta", "platforms"),
    "prerequisites": ("forgent_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.forgent.original_frontmatter,
        and merges metadata.forgent.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize forgent namespace
        oj_meta = dict(raw_metadata.get("forgent") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.forgent.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.forgent.* into canonical fields (these override
        # top-level mappings since they are explicit ForgentAI-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.forgent.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the forgent metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["forgent"] = oj_meta
