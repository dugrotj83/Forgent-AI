...
must NEVER be inside the ForgentAI source tree. ``resolve_spec_search_root``
walks up from this module's ``__file__`` looking for a ``pyproject.toml`` that
identifies the ForgentAI source root, then refuses to operate if the resolved
root is inside it. Defense in depth — if a user accidentally points
``FORGENT_HOME`` at the repo, the system fails loudly instead of silently
...
from forgent.core.paths import ConfigurationError, get_config_dir
from forgent.security.file_utils import secure_mkdir

# ``ConfigurationError`` is re-exported from ``forgent.core.paths`` (it used
...
def _find_source_root() -> Path | None:
    ...
    Returns the directory containing the ForgentAI ``pyproject.toml``, or
...
def _resolve_forgent_home() -> Path:
    """Resolve the ForgentAI home directory via the unified core resolver.

    Delegates to ``get_config_dir`` so spec-search honors the same env-aware
    resolution (FORGENT_HOME and XDG) as the rest of the framework.
    """
...
def resolve_spec_search_root() -> Path:
    ...
    The root is ``$FORGENT_HOME/learning`` (or ``~/.forgent/learning``
    by default). Raises ``ConfigurationError`` if the resolved path lies
    inside the ForgentAI source tree, to prevent dev artifacts from leaking
    into the repo.
    """
    home = _resolve_forgent_home()
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"FORGENT_HOME ({home}) is inside the source tree "
                f"({source_root}). Spec search refuses to write runtime "
                "artifacts inside the ForgentAI repo. Set FORGENT_HOME "
