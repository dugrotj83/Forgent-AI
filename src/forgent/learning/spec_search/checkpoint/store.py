...
A thin wrapper over a local git repository at ``<forgent_home>/.git``.
...
from forgent.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.forgent/`` (or ``$FORGENT_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the ForgentAI source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"ForgentAI source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@forgent.local")
            self._git("config", "user.name", "ForgentAI Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new forgent home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
