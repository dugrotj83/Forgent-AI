...
Uses GEPA's adapter pattern to bridge ForgentAI traces into GEPA's
...
from forgent.core.config import GEPAOptimizerConfig
from forgent.core.registry import LearningRegistry
from forgent.learning._stubs import AgentLearningPolicy
...
class ForgentAIGEPAAdapter:
    """Implements GEPA's adapter protocol for ForgentAI agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'forgent[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = ForgentAIGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "ForgentAIGEPAAdapter"]
