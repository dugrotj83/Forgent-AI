...
The wrapper adapts ForgentAI traces into ACE's
...
sidecar overlay under ``~/.forgent/learning/ace/<task>/`` for the
...
from forgent.core.config import ACEOptimizerConfig
...
class _TraceDataProcessor:
    """Adapter that exposes ForgentAI traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert ForgentAI trace records to ACE's sample dict shape."""
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": (
                    "ace not installed (pip install 'forgent[learning-ace]')"
                ),
            }
