"""Single point of contact between Python and the Rust ``forgent_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``forgent_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    """Return the ``forgent_rust`` module.
    ...
    """
    import forgent_rust  # type: ignore[import-untyped]

    return forgent_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``forgent_rust`` extension is importable.
...
def scan_result_from_json(json_str: str) -> object:
    ...
    from forgent.security.types import (
        ScanFinding,
        ScanResult,
        ThreatLevel,
    )
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from forgent.security.injection_scanner import (
        InjectionScanResult,
    )
    from forgent.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from forgent.tools.storage._stubs import RetrievalResult
