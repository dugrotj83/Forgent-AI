...
class OAuthProvider:
    ...
    # Filenames in ~/.forgent/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.forgent/connectors/`` and falls
    back to environment variables ``FORGENT_{NAME}_CLIENT_ID`` and
    ``FORGENT_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"FORGENT_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to ForgentAI.</p>"
                    b"</body></html>"
                )
