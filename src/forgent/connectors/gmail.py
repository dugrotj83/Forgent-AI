...
Uses OAuth 2.0 tokens stored locally (see :mod:`forgent.connectors.oauth`).
...
from forgent.connectors._stubs import BaseConnector, Document, SyncStatus
from forgent.connectors.google_auth import (
    GoogleAuthError,
)
from forgent.connectors.google_auth import (
    call_with_refresh as _call_with_refresh,
)
from forgent.connectors.oauth import (
    GOOGLE_ALL_SCOPES,
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `forgent connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `forgent connect gmail`"
            )
        return new
