"""Detect how Forgent AI was installed so we can show the right upgrade
command (and run the right upgrade command for ``forgent self-update``).

Three install paths are supported today:

- **PyPI** (``pip install forgent``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade forgent``.
- **uv tool** (``uv tool install forgent``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade forgent``.
...
We detect by inspecting ``forgent.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``forgent.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import forgent

        pkg_file = Path(forgent.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade forgent",
        )
