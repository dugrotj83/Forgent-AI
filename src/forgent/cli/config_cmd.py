"""``forgent config`` — configuration inspection commands."""
...
def _get_config_path(path: str | None) -> Path:
    ...
    if path:
        return Path(path)
    return Path(os.environ.get("FORGENT_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from forgent.core.config import detect_hardware, recommend_engine
...
@config.command("path")
def show_path() -> None:
    """Print the resolved Forgent AI directories (home, config, cache).

    All Forgent AI state lives under a single root, resolved in priority
    order: ``$FORGENT_HOME`` > ``$XDG_DATA_HOME/forgent`` >
    ``~/.forgent``. Use this to confirm where your data is stored after
    setting an override.
    """
    from forgent.core.paths import get_cache_dir, get_config_dir, get_config_path

    console = Console(stderr=True)
    home = get_config_dir()
    override = (
        "FORGENT_HOME"
        if os.environ.get("FORGENT_HOME")
        else "XDG_DATA_HOME"
        if os.environ.get("XDG_DATA_HOME")
        else "default (~/.forgent)"
    )
...
def set_config(key: str, value: str) -> None:
    """Set a configuration value (e.g. forgent config set engine.ollama.host URL)."""
    import tomlkit

    from forgent.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("FORGENT_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
