"""``forgent channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from forgent.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "forgent channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "forgent.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from forgent.connectors.retriever import (
            TwoStageRetriever,
        )
        from forgent.connectors.store import KnowledgeStore
        from forgent.engine.ollama import OllamaEngine
        from forgent.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from forgent.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from forgent.channels.imessage_daemon import stop_daemon
