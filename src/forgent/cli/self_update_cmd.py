"""`forgent self-update` — upgrade Forgent AI to the latest release.

Runs the right upgrade command for how the user installed Forgent AI:

- PyPI installs get ``pip install --upgrade forgent``.
- uv-tool installs get ``uv tool upgrade forgent``.
...
@click.command(
    "self-update",
    help=(
        "Upgrade Forgent AI to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo(f"Current Forgent AI version: v{current}")
    ...
    click.echo("\nUpgrade complete. Re-run `forgent --version` to confirm.")
