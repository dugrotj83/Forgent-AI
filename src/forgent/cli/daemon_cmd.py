"""``forgent start|stop|restart|status`` — daemon management commands."""
...
from forgent.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the Forgent AI server daemon."""
...
def start(
    ...
    """Start the Forgent AI server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'forgent stop' to stop it first, or 'forgent restart'.")
        sys.exit(1)
    ...
    # Build command to run forgent serve
    cmd = [sys.executable, "-m", "forgent.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Forgent AI server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Forgent AI server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Forgent AI server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Forgent AI server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
