...
Used by both ``install.sh`` (via ``forgent _bootstrap --write-config``)
and ``forgent init`` (so there is a single source of truth for the
...
import forgent
from forgent.core import config as _cfg
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``forgent _bootstrap --write-config``)
    and ``forgent init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# Forgent AI configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: forgent init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `forgent --help`
# ---------------------------------------------------------------------------
