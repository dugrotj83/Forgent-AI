"""``forgent optimize`` — LLM-driven configuration optimization CLI."""
...
def optimize_run(
    ...
    """Run an optimization loop over Forgent AI configuration."""
    ...
    if config_path is not None:
        try:
            from forgent.learning.optimize.config import load_optimize_config
        except ImportError:
    ...
    if data is not None:
        try:
            from forgent.learning.optimize.config import load_benchmark_specs
    ...
    try:
        ...
        from forgent.learning.optimize.config import load_objectives
        ...
        from forgent.learning.optimize.search_space import (
            DEFAULT_SEARCH_SPACE,
            build_search_space,
        )
        from forgent.learning.optimize.store import OptimizationStore
        from forgent.learning.optimize.trial_runner import (
        ...
        try:
            from forgent.evals.cli import _build_judge_backend
...
def skills(policy: str, min_traces: int, dry_run: bool) -> None:
    ...
    from forgent.learning.agents.skill_optimizer import SkillOptimizer
