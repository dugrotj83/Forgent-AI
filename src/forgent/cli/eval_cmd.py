"""``forgent eval`` — evaluation framework CLI commands."""
...
@click.option(
    ...
    type=click.Choice(
        ["forgent-direct", "forgent-agent", "hermes", "openclaw", "terminalbench-native"]
    ),
    help=(
        "Inference backend. For hermes/openclaw, also pass --base-url and "
        "--api-key (or set FORGENT_BACKEND_BASE_URL/FORGENT_BACKEND_API_KEY)."
    ),
)
@click.option(
    "--base-url",
...
def eval_run(
    ...
    if config_path is not None:
        ...
        try:
            from forgent.evals.cli import _run_single
        except ImportError:
    ...
    config = RunConfig(
        ...
        # FORGENT_BACKEND_BASE_URL/FORGENT_BACKEND_API_KEY.
