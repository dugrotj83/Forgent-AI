"""``forgent deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
from forgent.connectors.pipeline import IngestionPipeline
from forgent.connectors.store import KnowledgeStore
from forgent.connectors.sync_engine import SyncEngine
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from forgent.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from forgent.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from forgent.connectors.obsidian import ObsidianConnector

        return ObsidianConnector(vault_path=config.get("vault_path", ""))
    elif connector_id == "gmail_imap":
        from forgent.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from forgent.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from forgent.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from forgent.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from forgent.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.forgent/sync_state.db`` when empty.
