"""``forgent host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      forgent host mlx-community/Qwen2.5-7B-4bit --backend mlx
      forgent host Qwen/Qwen3-8B --backend vllm
      forgent host qwen3:8b --backend ollama
      forgent host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Forgent AI will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
