"""``forgent serve`` — OpenAI-compatible API server."""
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from forgent.engine.cloud import CloudEngine
    ...
    if len(multi_entries) > 1:
        from forgent.engine.multi import MultiEngine
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import forgent.tools  # noqa: F401  # trigger registration
            ...
            try:
                ...
                if AgentRegistry.contains(channel_agent):
                    ...
                    if getattr(_ch_cls, "accepts_tools", False):
                        ...
                        from forgent.mcp.loader import (
                        ...
                        # ForgentSystem below; we extend its lifetime by
    ...
    # block so the executor's ForgentSystem can reference it (#263).
    memory_backend = None
    ...
    memory_service = None
    try:
        from forgent.memory import build_memory_service
    ...
    if agent_manager is not None:
        try:
            ...
            from forgent.system import ForgentSystem
            from forgent.tools._stubs import ToolExecutor
    ...
    from forgent.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from forgent.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting Forgent AI API server[/green]\n"
