"""``forgent ask`` — send a query to the assistant."""
...
from forgent.cli._banner import print_banner
from forgent.cli._tool_names import resolve_tool_names
from forgent.cli.hints import hint_no_engine
...
from forgent.intelligence import (
    merge_discovered_models,
    register_builtin_models,
)
from forgent.telemetry.instrumented_engine import InstrumentedEngine
...
def _run_research(
    *,
    ...
    from forgent.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from forgent.connectors.embeddings import OllamaEmbedder
    from forgent.connectors.hybrid_search import HybridSearch
    ...
    # (so ``forgent ask --research "..." > out.md`` still gives a clean
...
def _run_agent(
    ...
    import forgent.agents  # noqa: F401
    from forgent.agents._stubs import AgentContext
    ...
    from forgent.mcp.loader import load_mcp_tools_from_config
    ...
    if config.agent.context_from_memory:
        try:
            from forgent.tools.storage.context import ContextConfig, inject_context
...
@click.option(
    ...
    default=None,
    help=(
        "Named persona dir under ~/.forgent/personas/<name>/ "
...
def ask(
    ...
    if capture_screen:
        try:
            from forgent.cli._screen import capture_screen_to_temp
    ...
    # the most common command (`forgent ask "..."`).
    ...
    if resolved is None:
        console.print(
            ...
            "  [cyan]forgent config set engine.ollama.host http://<remote-ip>:11434[/cyan]\n"
    ...
    from forgent.security import setup_security
    ...
    # Privacy guard: a screenshot/image is sensitive, and Forgent AI is
    ...
    if not no_context and config.agent.context_from_memory:
        try:
            from forgent.tools.storage.context import (
                ContextConfig,
                inject_context,
            )
