"""``forgent init`` — detect hardware, generate config, write to disk."""
...
from forgent.cli._bootstrap import detect_cloud_keys
from forgent.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``forgent init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     forgent ask "Hello"\n'
            "\n"
            "  Run `forgent doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]forgent scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-forgent first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_forgent: bool = False,
) -> None:
    """Detect hardware and generate ~/.forgent/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "forgent" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]forgent doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-forgent cold path: use the recommended engine non-interactively.
        if from_bare_forgent:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]forgent connect gdrive[/bold] to connect "
            "Google services, then [bold]forgent digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Forgent, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_forgent and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]forgent model pull {model}[/bold]"
                )
