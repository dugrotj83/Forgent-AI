"""``forgent doctor`` — run diagnostic checks on the Forgent AI installation."""
...
from forgent.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `forgent init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from forgent.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "forgent[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "forgent[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "forgent[energy-amd]", "AMD energy monitoring"),
        ("colbert", "forgent[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "forgent[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Forgent AI installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Forgent AI Doctor[/bold]")
    console.print()
