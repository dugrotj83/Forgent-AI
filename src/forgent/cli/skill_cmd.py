...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from forgent.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from forgent.skills.sources.openclaw import OpenClawResolver
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``forgent skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from forgent.skills.importer import SkillImporter
    from forgent.skills.parser import SkillParser
    from forgent.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.forgent/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: forgent skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.forgent/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="forgent-discover-dryrun-"))
        try:
