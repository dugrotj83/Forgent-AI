"""Config-driven fluent builder that wires up a ForgentSystem."""
...
from forgent.core.config import ForgentConfig, load_config
from forgent.core.events import EventBus, get_event_bus
...
from forgent.tools._stubs import BaseTool, ToolExecutor

logger = logging.getLogger(__name__)


class SystemBuilder:
    """Config-driven fluent builder for ForgentSystem."""

    def __init__(
        self,
        config: Optional[ForgentConfig] = None,
        *,
    ...
    def engine_instance(
        ...
        ``forgent eval --base-url``). ``build()`` health-checks the instance
    ...
    def build(self) -> ForgentSystem:
        """Construct a fully wired ForgentSystem."""
        config = self._config
    ...
    def _resolve_model(self, config: ForgentConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import forgent.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import forgent.channels  # noqa: F401 -- trigger registration
            from forgent.core.registry import ChannelRegistry
            from forgent.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from forgent.sandbox.runner import ContainerRunner
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: ForgentConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from forgent.core.config import DEFAULT_CONFIG_DIR
            from forgent.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from forgent.learning.training.lora import LoRATrainingConfig
            from forgent.traces.store import TraceStore
