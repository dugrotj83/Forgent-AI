...
use forgent_core::config::ForgentConfig;
...
pub fn discover_engines(config: &ForgentConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, ForgentAIError> {
    ...
    match key.as_str() {
        ...
        other => Err(ForgentAIError::Engine(
            forgent_core::error::EngineError::ModelNotFound(format!(
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = ForgentConfig::default();
