...
use forgent_core::error::ForgentAIError;
use forgent_core::{EventBus, EventType, GenerateResult, Message};
use forgent_engine::traits::{InferenceEngine, TokenStream};
    ...
    fn handle_findings(
        ...
        match self.mode {
            ...
            RedactionMode::Block => {
                ...
                Err(ForgentAIError::Security(
                    forgent_core::error::SecurityError::Blocked(format!(
...
mod tests {
    ...
    fn test_guardrails_block_mode() {
        ...
        let err = guardrails
            .generate(&[Message::user("Hi")], "mock", 0.7, 100, None)
            .unwrap_err();
        assert!(matches!(err, ForgentAIError::Security(_)));
    }
}
