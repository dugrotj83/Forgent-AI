...
pub struct PySecretScanner {
    inner: forgent_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: forgent_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: forgent_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: forgent_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: forgent_security::GuardrailsEngine<forgent_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => forgent_security::RedactionMode::Redact,
            "block" => forgent_security::RedactionMode::Block,
            _ => forgent_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: forgent_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<forgent_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = forgent_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: forgent_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: forgent_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: forgent_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: forgent_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: forgent_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: forgent_security::RateLimiter::new(
                forgent_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: forgent_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: forgent_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => forgent_security::TaintLabel::Pii,
            "secret" => forgent_security::TaintLabel::Secret,
            "user_private" => forgent_security::TaintLabel::UserPrivate,
            "external" => forgent_security::TaintLabel::External,
            _ => forgent_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &forgent_security::TaintSet::from_labels(&[taint_label]),
        );
    }
