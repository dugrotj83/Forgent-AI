...
use crate::RUNTIME;
use forgent_agents::OjAgent;
use forgent_engine::rig_adapter::RigModelAdapter;
use forgent_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(forgent_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(forgent_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(forgent_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&forgent_core::AgentContext>,
    ) -> Result<forgent_core::AgentResult, forgent_core::ForgentAIError> {
        match self {
...
fn make_adapter(engine_key: &str, model: &str) -> PyResult<DefaultAdapter> {
    let config = forgent_core::ForgentConfig::default();
    let engine = forgent_engine::get_engine_static(&config, Some(engine_key))
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = forgent_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(forgent_tools::ToolExecutor::new(None, None));
        let agent = forgent_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(forgent_tools::ToolExecutor::new(None, None));
        let agent = forgent_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = forgent_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = forgent_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(forgent_tools::ToolExecutor::new(None, None));
        let agent = forgent_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => forgent_agents::MemoryExtraction::Scratchpad,
            "structured_json" => forgent_agents::MemoryExtraction::StructuredJson,
            "none" => forgent_agents::MemoryExtraction::None,
            _ => forgent_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => forgent_agents::ObservationCompression::Truncate,
            "none" => forgent_agents::ObservationCompression::None,
            _ => forgent_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => forgent_agents::RetrievalStrategy::Keyword,
            "semantic" => forgent_agents::RetrievalStrategy::Semantic,
            "none" => forgent_agents::RetrievalStrategy::None,
            _ => forgent_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => forgent_agents::TaskDecomposition::Monolithic,
            "hierarchical" => forgent_agents::TaskDecomposition::Hierarchical,
            _ => forgent_agents::TaskDecomposition::Phased,
        };

        let monitor_config = forgent_agents::MonitorConfig {
        ...
        let agent = forgent_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: forgent_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: forgent_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
