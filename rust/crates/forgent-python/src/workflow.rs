...
pub struct PyWorkflowGraph {
    inner: forgent_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: forgent_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => forgent_workflow::NodeType::Tool,
            "condition" => forgent_workflow::NodeType::Condition,
            "parallel" => forgent_workflow::NodeType::Parallel,
            "loop" => forgent_workflow::NodeType::Loop,
            "transform" => forgent_workflow::NodeType::Transform,
            _ => forgent_workflow::NodeType::Agent,
        };
        let node = forgent_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = forgent_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<forgent_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(forgent_workflow::WorkflowBuilder::new(name)),
        }
    }
