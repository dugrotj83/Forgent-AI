...
use forgent_core::{AgentContext, AgentResult, ForgentAIError, ToolResult};
use forgent_tools::executor::ToolExecutor;
use regex::Regex;
...
impl<M: CompletionModel + 'static> OjAgent for NativeOpenHandsAgent<M> {
    ...
    async fn run(
    ...
    ) -> Result<AgentResult, ForgentAIError> {
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        forgent_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        forgent_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    ForgentAIError::Agent(forgent_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<forgent_engine::Engine>>;
