//! Error types for ForgentAI.

use thiserror::Error;

/// Top-level error type for all ForgentAI operations.
#[derive(Error, Debug)]
pub enum ForgentAIError {
...
mod tests {
    ...
    fn test_error_from_registry() {
        let e: ForgentAIError =
            RegistryError::DuplicateKey("foo".into(), "ToolRegistry").into();
        assert!(matches!(e, ForgentAIError::Registry(_)));
    }
