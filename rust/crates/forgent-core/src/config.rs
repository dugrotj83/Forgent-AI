...
//!
//! Rust translation of `src/forgent/core/config.py`.
...
/// Default config directory: `~/.forgent/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".forgent")
}

/// Default config file path: `~/.forgent/config.toml`
...
fn default_sandbox_image() -> String { "forgent-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.forgent/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<ForgentConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: ForgentConfig = toml::from_str(&content)?;
...
mode = "block"
"#;
        let cfg: ForgentConfig = toml::from_str(toml_str).unwrap();
