# Pearl reference oracle (ForgentAI Phase 0 deliverable)
...
If it succeeds, this host can mine Pearl using the ForgentAI `cpu-pearl` provider
...
- When Pearl bumps `py-pearl-mining` API: re-run the smoke test against the
  new ref pinned in `ForgentAI/src/forgent/mining/_constants.py`.
