...
struct BootPlan {
    ...
    /// ~/.forgent/config.toml so `forgent serve` picks it up.
    engine_host: Option<(String, String)>,
    /// Args appended after `uv run forgent serve --port <port>`.
...
fn boot_plan(cfg: &InferenceConfig, ram_gb: f64) -> BootPlan {
    match cfg.kind {
        ...
        SourceKind::Custom => {
            ...
            // `model` may be empty if the config is malformed; `forgent serve`
...
/// Find the ForgentAI project root (contains pyproject.toml).
/// Checks FORGENT_ROOT env var, walks up from the executable, then
/// probes common clone locations.
fn find_project_root() -> Option<std::path::PathBuf> {
    // 1. Explicit env var override
    if let Ok(root) = std::env::var("FORGENT_ROOT") {
    ...
    let home = home_dir();
    let direct = [
        format!("{home}/ForgentAI"),
        format!("{home}/projects/hazy/ForgentAI"),
        format!("{home}/projects/ForgentAI"),
        format!("{home}/src/ForgentAI"),
        format!("{home}/Documents/ForgentAI"),
        format!("{home}/Desktop/ForgentAI"),
        format!("{home}/Developer/ForgentAI"),
        format!("{home}/dev/ForgentAI"),
        format!("{home}/Code/ForgentAI"),
        format!("{home}/code/ForgentAI"),
        format!("{home}/repos/ForgentAI"),
        format!("{home}/github/ForgentAI"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for ForgentAI one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/ForgentAI without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("ForgentAI");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is ForgentAI (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("forgent")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + Forgent server child processes
// ---------------------------------------------------------------------------
...
/// Rolling buffer holding the most recent ~16 KB of forgent stderr.
///
/// Populated by a background drainer task spawned at boot so the pipe
/// never fills and back-pressures `forgent serve`; consumed by the boot
...
struct BackendManager {
    ...
    forgent: Option<ChildHandle>,
    forgent_stderr_tail: StderrTail,
}
...
impl Default for BackendManager {
    fn default() -> Self {
        Self {
            ollama: None,
            forgent: None,
            forgent_stderr_tail: Arc::new(Mutex::new(Vec::new())),
        }
    }
}
...
impl BackendManager {
    async fn stop_all(&mut self) {
        if let Some(ref mut h) = self.forgent {
            h.kill().await;
        }
        self.forgent = None;
...
/// Outcome of waiting for `forgent serve` to become healthy.
///
...
fn spawn_forgent_stderr_drainer(mut stderr: tokio::process::ChildStderr, tail: StderrTail) {
...
async fn read_forgent_stderr_tail(backend: &SharedBackend) -> String {
    let tail = backend.lock().await.forgent_stderr_tail.clone();
    let bytes = tail.lock().await.clone();
    String::from_utf8_lossy(&bytes).trim().to_string()
}

/// Poll `forgent serve` health, watching the child process state so we
/// never wait 10 minutes for a process that crashed in the first second.
async fn wait_for_forgent_health(
    url: &str,
    timeout: Duration,
    ...
    {
        Ok(c) => c,
        Err(_) => return ForgentStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    loop {
        ...
        let exit_status = {
            ...
            match mgr.forgent.as_mut() {
                Some(h) => h.child.try_wait().ok().flatten(),
                None => None,
            }
        };
        if let Some(status) = exit_status {
            let stderr = read_forgent_stderr_tail(backend).await;
            return ForgentStartResult::EarlyExit {
                code: status.code(),
                stderr,
            };
        }
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                if status.is_success() {
                    return ForgentStartResult::Ready;
                }
                if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                    ...
                    return ForgentStartResult::ServiceUnavailable(body);
                }
        ...
        if tokio::time::Instant::now() >= deadline {
            return ForgentStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
///
/// When the ForgentAI desktop binary is shipped as an AppImage, the AppImage
...
fn rust_toolchain_install_hint() -> &'static str {
    "The desktop app needs the Rust toolchain to build `forgent_rust`. \
...
fn format_extension_import_failure(root: &std::path::Path, stderr: &str) -> String {
    ...
    format!(
        "`forgent_rust` is still not importable after building. Last output:\n\n{}\n\n\
         Run these manually for the full build log:\n\n\
           cd {}\n\
           {}\n\
           uv run python -c \"import forgent_rust\"",
        if tail.is_empty() {
            "(no stderr output)"
        } else {
            &tail
...
async fn verify_forgent_rust_extension(
    root: &std::path::Path,
    uv_bin: &str,
) -> Result<(), String> {
    let mut cmd = tokio::process::Command::new(uv_bin);
    cmd.args(["run", "python", "-c", "import forgent_rust"])
    ...
    match cmd.output().await {
        ...
        Err(e) => Err(format!(
            "Could not verify `forgent_rust`: {}. Verify uv is installed at `{}`.",
            e, uv_bin
        )),
    }
}
...
fn port_owner_hint() -> String {
    if cfg!(target_os = "windows") {
        format!("netstat -ano | findstr :{}", FORGENT_PORT)
    } else {
        format!("lsof -i :{}", FORGENT_PORT)
    }
}
...
fn format_port_unavailable(port: u16, reason: &str) -> String {
    format!(
        "Port {} is not available: {}. Stop the process using that port or \
         change the ForgentAI port, then relaunch.\n\nTo identify it:\n  {}",
        port,
        reason,
        port_owner_hint(),
    )
...
fn check_forgent_port_available() -> Result<(), String> {
    match std::net::TcpListener::bind(("127.0.0.1", FORGENT_PORT)) {
        Ok(listener) => {
            drop(listener);
            Ok(())
        }
        Err(err) => Err(format_port_unavailable(FORGENT_PORT, &err.to_string())),
    }
}
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if plan.launch_ollama {
        ...
        // Point `forgent serve` at the user's endpoint by writing the engine
        // host into ~/.forgent/config.toml (the env var alone is shadowed by
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("ForgentAI");
        ...
        if target_path.exists() && !target_path.join("pyproject.toml").exists() {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "{} exists but is not a valid ForgentAI project. \
                 Remove it and relaunch, or set FORGENT_ROOT to the correct path.",
                clone_target,
            ));
            return;
        }
        ...
        {
            let mut s = status.lock().await;
            s.detail = "Downloading ForgentAI (first launch)...".into();
        }
        ...
        let clone_result = tokio::process::Command::new(&git_bin)
            .args([
                ...
                "--depth",
                "1",
                "https://github.com/dugrotj83/Forgent-AI.git",
                &clone_target,
            ])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::piped())
        ...
        match clone_result {
            Ok(child) => match child.wait_with_output().await {
                ...
                Ok(output) => {
                    ...
                    let mut s = status.lock().await;
                    s.error = Some(format!(
                        "Failed to download ForgentAI: {}. \
                         Clone manually: git clone https://github.com/dugrotj83/Forgent-AI.git {}",
                        stderr.trim(),
                        clone_target,
                    ));
                    return;
    ...
    // already started `forgent serve` in a terminal and then launched the
    ...
    // override from config instead of hard-coding FORGENT_PORT.
    {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        match client
            .get(format!("http://127.0.0.1:{}/health", FORGENT_PORT))
            .send()
            .await
        {
            ...
            Ok(resp) if resp.status() == reqwest::StatusCode::SERVICE_UNAVAILABLE => {
                ...
                s.error = Some(format!(
                    "An API server is already running on port {} but its \
                     ...
                     `forgent serve`, wait for it to finish loading and relaunch. \
    ...
    if let Err(err) = check_forgent_port_available() {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    // the user saw no error, the boot continued, `uv run forgent serve`
    // then ran in an under-provisioned venv, and the user waited the full
    // 600s health-check window before getting "Forgent server did not
    ...
    sync_cmd
        .args([
            ...
            // forgent_rust lives in a uv dependency group (not the published
    ...
    {
        let mut s = status.lock().await;
        s.detail = "Verifying Rust extension (forgent_rust)...".into();
    }
    if let Err(err) = verify_forgent_rust_extension(root, &uv_bin).await {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    match forgent_child {
        Ok(mut child) => {
            ...
            let mut mgr = backend.lock().await;
            let tail = mgr.forgent_stderr_tail.clone();
            mgr.forgent = Some(ChildHandle { child });
            drop(mgr);
            if let Some(stderr) = stderr_handle {
                spawn_forgent_stderr_drainer(stderr, tail);
            }
        }
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start forgent server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the ForgentAI repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    let server_url = format!("http://127.0.0.1:{}/health", FORGENT_PORT);
    match wait_for_forgent_health(&server_url, Duration::from_secs(600), &backend).await {
        ForgentStartResult::Ready => {}
        ForgentStartResult::ServiceUnavailable(body) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Forgent server is running but the inference engine is not available \
                 (HTTP 503). This usually means the configured model couldn't be loaded.\n\n\
                 Check the server logs, or run 'uv run forgent serve --port {}{}' \
        ...
        ForgentStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Forgent server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The ForgentAI repo is at {}\n\
        ...
        ForgentStartResult::Timeout => {
            let stderr = read_forgent_stderr_tail(&backend).await;
            let mut s = status.lock().await;
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Forgent server did not become ready within 10 minutes. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The ForgentAI repo is at {}\n\
                ...
                format!(
                    "Forgent server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
fn api_base() -> String {
    format!("http://127.0.0.1:{}", FORGENT_PORT)
}
...
#[tauri::command]
async fn run_forgent_command(args: Vec<String>) -> Result<String, String> {
    let uv_bin = resolve_bin("uv");

    let mut cmd_args = vec!["run".to_string(), "forgent".to_string()];
    cmd_args.extend(args.iter().cloned());

    let mut cmd = tokio::process::Command::new(&uv_bin);
    cmd.args(&cmd_args);
    // Run from the project root so `uv run forgent` resolves the ForgentAI
    // project regardless of the app's launch cwd. In a packaged install the
    // cwd isn't the checkout, so without this `forgent` isn't found and the
    ...
    if !is_serve {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("Failed to launch forgent: {}", e))?;
    ...
    // `forgent serve` is a long-running server that never exits. The old code
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("Failed to launch forgent serve: {}", e))?;

    let tail: StderrTail = Arc::new(Mutex::new(Vec::new()));
    if let Some(stderr) = child.stderr.take() {
        spawn_forgent_stderr_drainer(stderr, tail.clone());
    }
    ...
    let url = format!("http://127.0.0.1:{}/health", FORGENT_PORT);
    ...
    loop {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(format!(
                "forgent serve exited (code {:?}) before becoming healthy:\n{}",
                status.code(),
                stderr.trim()
            ));
        }
        if let Ok(resp) = client.get(&url).send().await {
            if resp.status().is_success() {
                ...
                return Ok(format!(
                    "forgent serve is ready on http://127.0.0.1:{}",
                    FORGENT_PORT
                ));
            }
        }
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "forgent serve did not become healthy on port {} within 120s.",
                FORGENT_PORT
            ));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
...
const SECURE_KEY_SERVICE: &str = "ForgentAI Cloud Keys";
...
async fn reload_cloud_keys(keys: Vec<(String, String)>) {
    let reload_url = format!("http://127.0.0.1:{}/v1/cloud/reload", FORGENT_PORT);
...
// ---------------------------------------------------------------------------
// Inference-source selection (~/.forgent/inference.json)
// ---------------------------------------------------------------------------
...
/// Path to the inference-source config (~/.forgent/inference.json).
...
/// Write the custom-endpoint host into ~/.forgent/config.toml so
/// `forgent serve` (which reads that file via load_config) points at it.
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("ForgentOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("ForgentOverlayPanel", sup).unwrap();
        ...
        if Class::get("ForgentOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("ForgentOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("ForgentOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("ForgentOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("ForgentOverlayPanel").unwrap();
        ...
        let hcls = Class::get("ForgentOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("ForgentOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit ForgentAI").build(app)?;
            ...
            #[cfg(target_os = "macos")]
            unsafe {
                native_overlay::create(include_str!("overlay.html"), FORGENT_PORT);
            }
        ...
        .expect("error while building ForgentAI Desktop")
        .run(move |_app, event| {
...
mod tests {
    ...
    fn failure_message_includes_exit_code_and_tail_and_hint() {
        let msg = format_uv_sync_failure(
            Path::new("/home/u/.forgent/src"),
            Some(2),
            "error: failed to resolve numpy==2.1.3",
        );
        assert!(msg.contains("exit 2"));
        assert!(msg.contains("/home/u/.forgent/src"));
    ...
    fn missing_rust_toolchain_message_names_cargo_and_installer() {
        ...
        assert!(msg.contains("forgent_rust"));
    ...
    fn uv_sync_rust_failure_mentions_toolchain() {
        let msg = format_uv_sync_failure(
            Path::new("C:\\Users\\me\\ForgentAI"),
            Some(1),
            "maturin failed: linker `link.exe` not found while building forgent-rust",
        );
        assert!(msg.contains("exit 1"));
    ...
    fn extension_import_failure_names_verification_command() {
        let msg = format_extension_import_failure(
            Path::new("C:\\Users\\me\\ForgentAI"),
            "ModuleNotFoundError: No module named 'forgent_rust'",
        );
        assert!(msg.contains("forgent_rust"));
        assert!(msg.contains(DESKTOP_UV_SYNC_COMMAND));
        assert!(msg.contains("uv run python -c \"import forgent_rust\""));
