...
const CONVERSATIONS_KEY = 'forgent-conversations';
const SETTINGS_KEY = 'forgent-settings';
const OPTIN_KEY = 'forgent-optin';
const OPTIN_NAME_KEY = 'forgent-display-name';
const OPTIN_EMAIL_KEY = 'forgent-email';
const OPTIN_ANONID_KEY = 'forgent-anon-id';
const OPTIN_SEEN_KEY = 'forgent-optin-seen';
...
interface Settings {
  theme: ThemeMode;
  apiUrl: string;
  // Local server API key (FORGENT_API_KEY). Sent as a Bearer token on
  // /v1 + /api requests so a key-protected `forgent serve` doesn't 401 the
