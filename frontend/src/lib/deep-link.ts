/**
 * deep-link.ts — Handler for forgent:// deep links in the Tauri desktop app.
 *
 ...
 *   forgent://research/{session_id}   → open a research session
 *   forgent://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `forgent://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `forgent://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `forgent:` scheme.
 *
 * @example
 * parseDeepLink("forgent://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("forgent://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "forgent:") return null;

    // The URL constructor treats "forgent://research/abc123" such that
